...
#define PROGRAM_VERSION "Program:: $Date: 2010/03/04 21:17:27 $ $Revision: 1.2 $"
using namespace std;
...
std::string Program::usage("none available");
std::string Program::description("none available");

/**
 * the system keywords, known to all programs
 */
KeyTabEntry Program::system_[] = {
  { "logf",      "(none)", "s", "Logfile prefix"},
  { "logd",           ".", "s", "Logfile directory"},
...
int Program::initialize(void)
{
  ...
  try {
    ...
    if(!isDefault("logf")) {
      Logger::setLogFilePrefix(getParameter("logf"));
      Logger::setLogFileDirectory(getParameter("logd"));
      Logger::openLogFile();
    }
...
int Program::run(int argc, char* argv[])
{
  ...
  try  {
    ...
    if (returnValue) 
      return 0;                 // nothing to do after all ... return
				// right now
  } catch (...) {
    returnValue = -1;
  }
  ...
  try  {
    ...
    ReportSimpleError(err.what());
    returnValue = -1;
  } catch(...) {
    ReportError("Caught an unknown Exception");
    returnValue = -1;
  }
 ...
 */
string Program::getParameter (string key) 
{
  int i = findKey(key);
 ...
 */
int Program::getiParameter (string key) 
{
  string sp = getParameter(key);
 ...
 */
double Program::getdParameter (string key) 
{
  string sp = getParameter(key);
 ...
 */
bool Program::getbParameter (string key) 
{

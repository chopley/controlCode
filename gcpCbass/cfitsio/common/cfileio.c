...
#define MAX_PREFIX_LEN 20  /* max length of file type prefi5Bx (e.g. 'http://') */
...
static fitsdriver driverTable[MAX_DRIVERS];  /* allocate driver tables */
...
*/
{
    ...
    *fptr = 0;              /* initialize null file pointer */
    ...
    if (*extspec)
    {
      ...
      fprintf(stdout, "nr is %d %d extspec is: %s %s %d\n", sizeof(imagecolname), sizeof(rowexpress), 
	      extspec, extname, extnum);

      ffexts(extspec, &extnum, 
	     extname, &extvers, &movetotype, imagecolname, rowexpress, status);

       fprintf(stdout, "extspec is: %s %s %d\n", extspec, extname, extnum);

...
*/
{
    ...
    if (tcode == TSTRING)    /* setup for ASCII tables */
    {
      ...
      /*
       * This just defaulted to reading ffasfm, even though this
       * routine is supposed to be safe for binary table reading. I've
       * added a switch to use ffbnfm if the HDU is binary, so that
       * TFORM words like '8A' don't crash it.
       */
      switch (hdutype) {
      case ASCII_TBL:
	...
	{
	  long repeat;
	  ffbnfm(tform, &xcode, &repeat, &xwidth, status); 
	}
	break;
      default:
	ffasfm(tform, &xcode, &xwidth, &decimals, status); 

#ifndef GCP_RECEIVER_XMLDATAFRAMEMANAGER_H
#define GCP_RECEIVER_XMLDATAFRAMEMANAGER_H

/**
 * @file XMLDataFrameManager.h
 * 
...
namespace gcp {
  namespace receiver {
	...
	public:
	  ...
	  RegisterList(RegMapDataFrameManager* frame, 
	...
	private:
	  ...
	  bool addMapped(char* boardName, char* regName)
	    {
	      RegMapBlock* regBlock = NULL;
	      try {
		regBlock = frame_->findReg(boardName, regName);
	      } catch (...) {
		ReportError("Board" << boardName << " register " 
			    << regName << " not found.");
	      }
	...
	public:
	  ...
	  void add(char* boardName, char* regName) 
	    {
	      if(addMapped(boardName, regName)) {
		minMappedAddress_++;
	      }
	    }
                
	  void add(char* boardName, char* regName, unsigned int address)
	    {
		...
		return;
	      ...
	      addMapped(boardName, regName);
	    }	
                
	  void add(char* boardName, char* regName, std::string xmlTag)
	    {
	  ...
	  void write() 
		...
		frame_->writeRegNoLock(*regIter++, static_cast<valueType*>(&(*static_cast< std::vector<valueType>* >(*bufIter++))[0]));
	      }
	...
	template<class dataType> 
	  void add(RegisterList<dataType>* registers,
		   char* boardName, char* regName, std::string xmlTag, 
		   std::string dataType) {

	  registers->add(boardName, regName, xmlTag);

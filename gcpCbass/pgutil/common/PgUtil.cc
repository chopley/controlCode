 ...
 */
int PgUtil::v_lplot(int narr, float xarr[], float yarr[], char xlab[], char ylab[],
		    char title[])
{
  int i;
  char value[20];
 ...
 */
  ...
  if(docurs) {
	...
	break;
      ...
      if(redisp)
      	if(!cancel) 
	  v_ldraw(narr,xarr,yarr,xlab,ylab,title);
      if(read) 
	cpgband(B_NORM, 0, xpos[0], ypos[0], &xpos[0], &ypos[0], &key);
      read = 1;
      if(islower((int) key))
 ...
 *
 */
int PgUtil::v_ldraw(int narr, float xarr[], float yarr[], char xlab[], char ylab[], char title[])
{
  int i;
  float x1,x2;
  float ypts[5],xpts[5];

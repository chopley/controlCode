...
std::vector<RegInfo> cacheRegInfo(ArchiveReader& reader, unsigned nFrame)
{
  std::vector<RegInfo> regInfo;
  RegInfo info;
  std::vector<ArchiveReader::ArchiveRegister>& regs = reader.getRegs();
  std::vector<RegDescription>& regDescs = reader.getRegDescs();

  for(unsigned iReg=0; iReg < regs.size(); iReg++) {

    ArchiveReader::ArchiveRegister& reg = regs[iReg];

    info.desc_       = regDescs[iReg];
	       ...
	       std::vector<RegInfo>& regInfo)
{
  ...
  for(unsigned i=0; i < regInfo.size(); i++) {
    ArchiveReader::ArchiveRegister& reg = regs[i];

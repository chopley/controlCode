...
static int nim_read_sizes(NetImMonitor *nim, NetReadStr *nrs, int *nrs_size,
			 int *nss_size);
...
static NetImMonitor *new_NetImMonitor(char *host)
{
  ...
  int nrs_size;    /* The size of the network input buffer */
  int nss_size;    /* The size of the network output buffer */
/*
 ...
 * Input/Output:
 *  nrs_size     int *  The size required for the network input stream
 *                       will be recorded in *nrs_size.
 *  nss_size     int *  The size required for the network output stream
			 ...
			 int *nss_size)
{
  ...
  if(net_start_get(nrs->net, &opcode) ||
     opcode != IMC_SIZE_MSG ||
     net_get_int(nrs->net, 1, (unsigned int* )nrs_size) ||
     net_get_int(nrs->net, 1, (unsigned int* )nss_size) ||
     net_end_get(nrs->net)) {
...
static IMS_READ_IMAGE(nim_read_image)
{
  ...
  unsigned short *channel = ims_get_channel(ims);
  ...
  if(net_get_short(net, 1, channel) || 
     net_get_short(net, GRABBER_IM_SIZE, image)) 
    return IMS_READ_ENDED;

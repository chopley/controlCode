 ...
 */
SourceCatalog *new_SourceCatalog(bool update=false);
...
namespace gcp {
  namespace control {
    ...
    enum SourceType {
      ...
      SRC_CENTER,        /* A source at fixed azimuth and elevation which should be tracked */
    ...
    union SourceUnion {
      ...
      CenterSource center;  /* A source at fixed azimuth and elevation */
 ...
 * recursively).
 */
int get_SourceId(gcp::control::Source* src, int resolve, gcp::control::SourceId *id);
 ...
 */
int get_Source_number(gcp::control::Source* src, int resolve);

/*
 * Use the following functions to add sources to the catalog.
 * Each function returns NULL on failure.
 */
gcp::control::Source* add_J2000Source(SourceCatalog *sc, char *name, double ra, 
				    double dec, float pmra, float pmdec);
gcp::control::Source* add_EphemSource(SourceCatalog *sc, char *name, char *file);
gcp::control::Source* add_FixedSource(SourceCatalog *sc, char *name, 
				    unsigned axis_mask, double az, double el, 
				    double dk);
gcp::control::Source* add_CenterSource(SourceCatalog *sc, char *name, 
				    unsigned axis_mask, double az, double el, 
				    double dk);
gcp::control::Source* add_AliasSource(SourceCatalog *sc, char *name, char *source);
 ...
 * field.
 */
gcp::control::Source* find_SourceByName(SourceCatalog *sc, char *name);
gcp::control::Source* find_SourceByNumber(SourceCatalog *sc, int number);
...
typedef struct {
  ...
  double lst;         /* The corresponding Local Apparent Sidereal Time, */
                      /*  measured in radians. */
  double ra,dec;      /* The topocentric Right Ascension and Declination */
 ...
 */
int source_info(SourceCatalog *sc, gcp::control::Site *site, 
		gcp::control::Source* src, 
 ...
 * The returned time limits are specified in TT.
 */
int get_ephem_window(SourceCatalog *sc, gcp::control::Source* src, double tt,
		     gcp::control::CacheWindow *win);
 ...
 */
int get_ephem_QuadData(SourceCatalog *sc, gcp::control::Source* src, double tt,
		       gcp::control::QuadData *ra, 
		       gcp::control::QuadData *dec, 
		       gcp::control::QuadData *dist);
 ...
 */
int precess_J2000_source(SourceCatalog *sc, gcp::control::Source* src, double tt,
			 double *ra, double *dec);

/*
 * Get the coordinates of a fixed source.
 */
int describe_fixed_source(gcp::control::Source* src, SourceAzElPa *coord,
			  unsigned *axis_mask);

/*
 * Get the coordinates of a center source.
 */
int describe_center_source(gcp::control::Source* src, SourceAzElPa *coord,
			  unsigned *axis_mask);

...
static Source *read_EphemSource(SourceCatalog *sc, InputStream *stream,
				char *name);
static void registerEphemSource(SourceCatalog* sc, char* name, char* file);
...
static Source *new_CenterSource(SourceCatalog *sc, char *name,
			       unsigned axis_mask,
			       double az, double el, double dk);
static Source *read_CenterSource(SourceCatalog *sc, InputStream *stream,
				char *name);
static Source *bad_CenterSource(InputStream *stream, char *name, char *column);
static int write_CenterSource(SourceCatalog *sc, OutputStream *stream,
			    Source *src);
static int CenterSource_info(SourceCatalog *sc, Site *site, CenterSource *center,
			    double horizon, unsigned options, SourceInfo *info);
...
static HORIZONS_HANDLER(ephemSourceUpdateHandler);
...
struct SourceCatalog {
  ...
  FreeList *center_mem;       /* Memory for az/el sources */
  ...
  gcp::util::HorizonsCommunicator* horizonsCommunicator_;
};
...
/*.......................................................................
 * Add a center-position source to a source catalog.
 *
 * Input:
 *  sc   SourceCatalog *   The source catalog to which to add the source.
 *  name          char *   The name of the source.
 ...
 *  return      Source *   The new source, or NULL on error.
 */
Source *add_CenterSource(SourceCatalog *sc, char *name, unsigned axis_mask,
			double az, double el, double dk)
{
/*
 * Create the source.
 */
  Source *src = new_CenterSource(sc, name, axis_mask, az, el, dk);
  if(!src)
    return NULL;
/*
 * Add the source to the catalog.
 ...
 */
  ...
  if(sym) {
    DBPRINT(true, Debug::DEBUGANY, "Source: " << src->id.name << " already exists, of type: " << src->id.type);
...

{
  ...
  // Don't check deck angle for experiments like BICEP, which have unlimited travel
 ...
 * Allocate the container.
 */
  src = (Source* )new_FreeListNode("new_CenterSource", sc->center_mem);
/*
 * Before attempting any opertion that might fail, initialize the
 * source at least up to the point at which it can safely be passed
 * to del_Source().
 */
  ini_SourceId(src, SRC_CENTER);
  src->center.axis_mask = axis_mask;
  src->center.az = axis_mask & Axis::AZ ? az : 0.0;
  src->center.el = axis_mask & Axis::EL ? el : 0.0;
  src->center.dk = axis_mask & Axis::PA ? dk : 0.0;
/*
 * Domain check and record the name of the source.
 */
  if(set_SourceName(src, name))
 ...
 */
  ...
  // If the ephemeris container is empty, read from the ephemeris file
...
static Source *del_Source(SourceCatalog *sc, Source *src)
{
  if(src) {
    ...
    case SRC_FIXED:
      ...
      break;
    case SRC_CENTER:
      src = (Source* )del_FreeListNode("del_Source", sc->center_mem, src);
      break;
    case SRC_ALIAS:
 ...
 */
SourceCatalog *new_SourceCatalog(bool update)
{
 ...
 */
  ...
  sc->center_mem = (FreeList* )new_FreeList("new_SourceCatalog", 
			       sizeof(CenterSource), 20);
  if(!sc->center_mem)
    return del_SourceCatalog(sc);
 ...
 */
  ...
  if(update) {
    sc->horizonsCommunicator_ = new gcp::util::HorizonsCommunicator(30);
    sc->horizonsCommunicator_->spawn();
    sc->horizonsCommunicator_->addHandler(ephemSourceUpdateHandler, sc);
  }
 ...
 */
    ...
    sc->center_mem = del_FreeList("del_SourceCatalog", sc->center_mem, 1);
    sc->alias_mem = del_FreeList("del_SourceCatalog", sc->alias_mem, 1);

    if(sc->horizonsCommunicator_) {
      delete sc->horizonsCommunicator_;
      sc->horizonsCommunicator_ = 0;
    }
 ...
 */
    if(input_word(stream, 0, 1)) {
      return input_error(stream, 1,
	    "Missing source type. Should be J2000, EPHEM, FIXED, CENTER, or ALIAS.\n");
    };
/*
 ...
 */
	...
	return 1;
      ...
      // Read an ephemeris source.  If the Horizons updater is
      // non-NULL, we ignore the return value, since an ephemeris may
      // not yet exist for the source but may be updated later, in
      // which case the source will then be added to the catalog

    case SRC_EPHEM:
      if(!read_EphemSource(sc, stream, id.name)) {
	if(sc->horizonsCommunicator_ == 0)
	  return 1;
      }
      break;
    case SRC_FIXED:
	...
	return 1;
      break;
    case SRC_CENTER:
      if(!read_CenterSource(sc, stream, id.name))
	return 1;
      break;
    case SRC_ALIAS:
 ...
 */
	...
	break;
      case SRC_CENTER:
	if(write_CenterSource(sc, stream, src))
	  return 1;
	break;
      case SRC_ALIAS:
				...
				char *name)
{
  ...
  // Register the source with the ephemeris updater.  We do this first
  // in case the ephemeris doesn't (yet) exist, in which case
  // new_EphemSource() below will return NULL.

  try {
    registerEphemSource(sc, name, stream->work);
  } catch(...) {
    return NULL;
  }
...
/**.......................................................................
 * Register an ephemeris to be watched for updates
 */
static void registerEphemSource(SourceCatalog* sc, char* name, char* file)
{
  // If we are updating source ephemerides, add this source to the
  // list of sources whose ephemerides should be registered with the
  // Horizons communicator

  if(sc->horizonsCommunicator_) {
    sc->horizonsCommunicator_->registerEphemeris(name, file);
  }
} 
				...
				char *name)
{
  ...
  if(stream->nextc=='*') {
    az = 0.0;
    if(input_skip_space(stream, 1, 1))
      return bad_CenterSource(stream, name, "Azimuth");
  } else {
    if(input_sexagesimal(stream, 0, &az))
      return bad_CenterSource(stream, name, "Azimuth");
    axis_mask |= Axis::AZ;
  ...
  if(stream->nextc=='*') {
    el = 0.0;
    if(input_skip_space(stream, 1, 1))
      return bad_CenterSource(stream, name, "Elevation");
  } else {
    if(input_sexagesimal(stream, 0, &el))
      return bad_CenterSource(stream, name, "Elevation");
    axis_mask |= Axis::EL;
  ...
  if(stream->nextc == '*') {
    dk = 0.0;
    if(input_skip_space(stream, 1, 1))
      return bad_CenterSource(stream, name, "Deck-angle");
 ...
 */
  if(axis_mask == 0) {
    input_error(stream, 1,
		"Center source %s doesn't specify any axis positions.\n",
		name);
    return NULL;
  };
/*
 * Instantiate the source.
 */
  src = new_CenterSource(sc, name, axis_mask, az, el, dk);
  if(!src)
    return NULL;
/*
 * Add the source to the catalog.
...
/*.......................................................................
 * This is a private syntax-error return function for read_CenterSource().
 *
 * Input:
 *  stream  InputStream *  The stream in which the syntax error was found.
 *  name           char *  The name of the source that couldn't be read.
...
static Source *bad_CenterSource(InputStream *stream, char *name, char *column)
{
  input_error(stream, 1, "The %s of center source '%s' is invalid.\n",
	      column, name);
  input_error(stream, 1, "Use: CENTER name hh:mm:ss.s dd:mm:ss.s +dd:mm:ss.s\n");
  return NULL;
}
 ...
 *  sc      SourceCatalog *   The catalog to add the source to.
 *  stream   OutputStream *   The stream to write to.
 *  src            Source *   The Center source to be output.
 * Output:
 *  return            int     0 - OK.
 *                            1 - Error.
 */
 ...
 * Write the azimuth field.
 */
  if(src->center.axis_mask & Axis::AZ ?
     output_sexagesimal(stream, "", 12, 0, 3, src->center.az * rtoh) :
     write_OutputStream(stream, "*"))
    return 1;
/*
 * Write a field separator.
 ...
 * Write the elevation field.
 */
  if(src->center.axis_mask & Axis::EL ?
     output_sexagesimal(stream, "", 12, 0, 2, src->center.el * rtod) :
     write_OutputStream(stream, "*"))
    return 1;
/*
 * Write a field separator.
 ...
 * Write the azimuth field.
 */
  if(src->center.axis_mask & Axis::PA ?
     output_sexagesimal(stream, "", 12, 0, 2, src->center.dk * rtod) :
     write_OutputStream(stream, "*"))
    return 1;
  return 0;
}
 ...
 */
  ...
  case SRC_FIXED:
    ...
    break;
  case SRC_CENTER:
    if(CenterSource_info(sc, site, &src->center, horizon, options, info))
      return 1;
    break;
  case SRC_J2000:
...
/*.......................................................................
 * Return topocentric information about a center source.
 *
 * Input:
 *  sc   SourceCatalog *  The parent source catalog of the source.
 *  site          Site *  The location of the observer.
 *  center  CenterSource *  The source to examine.
 *  horizon     double    The elevation of the horizon to use for
 *                        rise/set calculations (radians).
 *  options   unsigned    A bitwise union of SrcInfoOpt inquiry options.
 * Input/Output:
 ...
 * Fill in as much of the information as possible.
 */
  info->axis_mask = center->axis_mask;
  info->coord.az = center->az;
  info->coord.el = center->el;
  info->coord.pa = center->dk;
  info->rates.az = 0.0;
  info->rates.el = 0.0;
  info->rates.pa = 0.0;
  info->sched = center->axis_mask & Axis::EL && center->el < horizon ?
    SRC_NEVER_RISES : SRC_NEVER_SETS;
  info->rise = 0.0;
  info->set = 0.0;
/*
 * Optionally compute the topocentric Right Ascension and Declination that
 * correspond to the center azimuth and elevation.
 */
  if(options & SIO_EQUAT) {
    horizon_to_equatorial(info->lst, site, sin(center->el), cos(center->el),
			  sin(center->az), cos(center->az),
			  &info->ra, &info->dec);
  };
  return 0;
}
 ...
 * horizontal coordinates to generate the almanac.
 */
...
/*.......................................................................
 * Get the coordinates of a center source. Note that an error will be
 * returned if the source is not a center source.
 *
 * Input:
 *  src          Source *  The center source to describe.
 * Input/Output:
 *  coord  SourceAzElPa *  The horizon coordinates of the source. 
 *  axis_mask  unsigned *  A bitwise union of source.h::SourceAxes enumerators
 *                         specifying which of the following axes are
 ...
 *                         1 - Error.                  
 */
int describe_center_source(Source *src, SourceAzElPa *coord, unsigned *axis_mask)
{
  CenterSource *center;   /* The type-specific member of 'src' */
/*
 * Check the arguments.
 */
  if(!src || !coord) {
    lprintf(stderr, "describe_center_source: NULL argument(s).\n");
    return 1;
  };
/*
 * Resolve source aliases.
 ...
 * Make sure that the right source type has been provided.
 */
  if(src->id.type != SRC_CENTER) {
    lprintf(stderr, "describe_center_source: Not a center source.\n");
    return 1;
  };
/*
 * Get the type-specific version of 'src'.
...
HORIZONS_HANDLER(ephemSourceUpdateHandler)
{
  if(!error) {
    SourceCatalog* sc = (SourceCatalog*)args;
    add_EphemSource(sc, (char*)srcName.c_str(), (char*)fileName.c_str());
  }
}

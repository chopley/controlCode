 ...
 */
ArrayMap::ArrayMap(void* vtmp, bool addDefaults)
{
  ...
  if(arraytmp==NULL) {
    ThrowError("NULL array template");
  };

  if(arraytmp->templates == NULL || arraytmp->ntemplate == 0) {
    ThrowError("Empty register map array");
  };
  ...
  // Create free-lists for the register symbol tables.

  hash_mem = new_HashMemory(200, 1000);

  if(!hash_mem) {
    ThrowError("new_HashMemory failed.");
  };
  
  // Create a symbol table of board names.

  hash = new_HashTable(hash_mem, 100, IGNORE_CASE, NULL, 0);
  ...
  // Resize the vector to accommodate the register map descriptions.
  ...
  for(i=0; i < nregmap; i++) {
    ...
    regmaps[i] = new ArrRegMap(this, regtemp, i, addDefaults);
  }
}
...
ArrayMap *del_ArrayMap(ArrayMap *arraymap)
{
  ...
  if(arraymap) {
    if(--arraymap->ref_count > 0) {
      return NULL;
    } else {
      for(int i=0; i < arraymap->regmaps.size(); i++) {
...
ArrayMap::~ArrayMap()
{
  ...
  // Now that all of the hash tables have been destroyed, delete the
  // free-lists from which they were created.
  
  hash_mem = del_HashMemory(hash_mem, 1);
#endif
}
...
ArrRegMap* find_ArrRegMap(ArrayMap *arraymap, string regmap_name)
{
  ...
  return regmap;
#else
  std::map<std::string, ArrRegMap*>::iterator slot;

  slot = arraymap->mapOfRegmaps_.find(regmap_name);

  if(slot == arraymap->mapOfRegmaps_.end()) 
    return NULL;
  else {
    return slot->second;
  }
...
std::vector<ArrRegMap*> ArrayMap::matchArrRegMap(string regExpStr)
{
  ...
  gcp::util::RegExpParser parser(regExpStr);
		      ...
		      RegAspect aspect, ArrRegMapReg *arreg)
{
  ...
  reg->block = block;
  reg->index = index;
  reg->slot  = (blk->slot_ < 0) ? -1 : (int)(blk->slot_ + reg->index * size);
			       ...
			       int extend, ArrRegMapReg *arreg)
{
  ...
  if(stream->nextc != '.') {
    std::cout  << "regmap_name is: " << regmap_name << std::endl;
    input_error(stream, tell,
		"Missing period after register-map (1) name.\n");
    return REG_INVALID;
  };
  ...
  if(stream->nextc == '.') {
    ...
    if(i >= (int)REG_NASPECT) {
      input_error(stream, tell,
		  "\"%s\" is not a recognized register attribute.\n",
		  stream->work);
      return REG_INVALID;
    };
  };
  ...
  if(regmap && index2 >= (int)nmax) {
    ...
    return index1 >= (int)nmax ? REG_UNKNOWN : REG_INVALID;
  };
  ...
  if(arreg) {
    ...
    arreg->regmap    = arregmap ? (int)arregmap->number : -1;
    arreg->reg.board = regmap ? (int) board->number : -1;
    arreg->reg.block = regmap ? (int) block->number_ : -1;
    arreg->reg.index = index1;

    arreg->reg.slot = (!regmap || block->slot_ < 0) ? -1 :
      (int)(block->slot_ + index1 * size);
			...
			ArrRegMapReg *arreg)
{
  ...
  case REG_OUTPUT_ELEMENT:
    if(blk->nreg_ > 1) {
	 ...
	 output_uint(stream, OUT_DECIMAL, "", 0, 0, reg->index) ||
	 write_OutputStream(stream, "]"))
	return 1;
    };
    break;
  case REG_OUTPUT_RANGE:
	...
	if(write_OutputStream(stream, "[") ||
	   output_uint(stream, OUT_DECIMAL, "",0,0, reg->index) ||
	   write_OutputStream(stream, "-") ||
	   output_uint(stream, OUT_DECIMAL, "",0,0, reg->index+reg->nreg-1) ||
	   write_OutputStream(stream, "]"))
	  return 1;
      };
    };
...
/**.......................................................................
 * Find the named board.  If doThrow is true, throw an error if the
 * register wasn't found.
 */
RegMapBoard* ArrayMap::findArrayMapBoard(string regmap_name, 
					 string board_name,
					 bool doThrow)
{
  RegMap* regmap = findRegMap(regmap_name, doThrow);
  ...
  if(doThrow && brd==0) 
    ThrowError("Lookup of " << board_name << " failed");
 ...
 * the register wasn't found.
 */
RegMap* ArrayMap::findRegMap(std::string regmap_name, bool doThrow)
{
  ...
  if(arregmap==0) {
    ...
    if(doThrow) {
      ThrowError("Unable to find register map: " << regmap_name);
    } else {
      return NULL;
    }
 ...
 */
ArrRegMap::ArrRegMap(ArrayMap* parent, void* vregtemp, unsigned iRegMap, 
		     bool addDefaults)
{
  RegTemp* regtemp = (RegTemp*)vregtemp;
  using gcp::util::LogStream;
  ...
  regmap = new RegMap(regtemp->regtemplate, addDefaults);
  ...
  if(!new_HashSymbol(parent->hash, name, 0, 0, this, 0)) 
    ThrowError("Bad name for register map " << iRegMap);
#else

  parent->mapOfRegmaps_[name] = this;
						      ...
						      CoordRange& range)
{
  ...
  if(range.nAxis() == 0) {
    ...
    for(unsigned iBoard=start; iBoard < stop; iBoard++) {
      std::ostringstream os;
      os << regExpStr << iBoard;
      gcp::util::RegExpParser parser(os.str());
...
void ArrayMap::print(bool archivedOnly)
{
  for(unsigned iRegMap=0; iRegMap < regmaps.size(); iRegMap++) {
    ArrRegMap* regMap = regmaps[iRegMap];
    for(unsigned iBrd=0; iBrd < regMap->regmap->boards_.size(); iBrd++) {
      RegMapBoard* brd = regMap->regmap->boards_[iBrd];
      for(unsigned iBlk=0; iBlk < brd->blocks.size(); iBlk++) {
	RegMapBlock* blk = brd->blocks[iBlk];

	if(!archivedOnly || !(blk->flags_ & REG_EXC)) {
	  //COUT(regMap->name << "." << brd->name << "." << blk->name_ << *blk->axes_);
	}
...
std::vector<std::string> ArrayMap::getRegisters(bool archivedOnly)
{
  ...
  for(unsigned iRegMap=0; iRegMap < regmaps.size(); iRegMap++) {
	...
	if(!archivedOnly || !(blk->flags_ & REG_EXC)) {
	  os.str("");
	  os << regMap->name << "." << brd->name << "." << blk->name_;
	  regs.push_back(os.str());
	}

...
static int format_integer(OutputStream *stream, OutputBase base,
			  ...
			  int negative, unsigned int ulval);
...
{
  // GSG - Cynthia and I suspect this should get initialized to NULL, right Erik?
 ...
 */
  // debugging
  //fprintf(stderr, "mallocing stream (%d)\n", stream);
  //if (stream) free(stream);
  stream = (OutputStream *) malloc(sizeof(OutputStream));
  //stream = (OutputStream *) calloc(1,sizeof(OutputStream));
  if(!stream) {
 ...
 */
  // GSG another attempt to fix the memory leak
  //if (fdata) free(fdata);
 ...
 */
  ...
  // fprintf(stderr, ">> Allocating sdata\n");
  if(!sdata) {
 ...
 */
  ...
  if(open_OutputStream(stream, sdata, write_StringOutput, del_StringOutput)) {
    sdata = (StringOutput* )del_StringOutput(sdata);
    //fprintf(stderr, ">> Deallocating sdata\n");
    return 1;
  };
  //fprintf(stderr, "\nWARNING: did not deallocate sdata?\n\n");
  return 0;
}
 ...
 *  lval            int     The value to be written.
 * Output:
 *  return           int     0 - OK.
 *                           1 - Error.
 */
int output_int(OutputStream *stream, OutputBase base, char *flags,
		int width, int precision, int lval)
{
  unsigned int ulval;   /* The positive version of lval */
  int negative;          /* True if lval is negative */
/*
 * Check arguments.
 */
  if(!stream || !flags) {
    lprintf(stderr, "output_int: NULL argument(s).\n");
    return 1;
  };
  switch(base) {
  case OUT_BINARY:
  ...
  case OUT_HEX:
    break;
  default:
    lprintf(stderr, "output_int: Unknown radix.\n");
    return 1;
  };
/*
...
/*
 * Delegate work to a function that is shared with output_uint().
*/
  return format_integer(stream, base, flags, width, precision, negative, ulval);
}

/*.......................................................................
 * Write an unsigned int int to an output stream, in one of a variety of
 * number bases.
 *
 * Input:
 *  stream  OutputStream *   The stream to write to.
 ...
 *                           1 - Error.
 */
int output_uint(OutputStream *stream, OutputBase base, char *flags,
		int width, int precision, unsigned int ulval)
{
/*
 * Check arguments.
 */
  if(!stream || !flags) {
    lprintf(stderr, "output_uint: NULL argument(s).\n");
    return 1;
  };
  switch(base) {
  case OUT_BINARY:
  ...
  case OUT_HEX:
    break;
  default:
    lprintf(stderr, "output_uint: Unknown radix.\n");
    return 1;
  };
/*
 * Delegate work to a function that is shared with output_int().
 */
 ...
 *  ulval           int     The positive version of the value to be
 *                           written.
 * Output:
 *  return           int     0 - OK.
 *                           1 - Error.

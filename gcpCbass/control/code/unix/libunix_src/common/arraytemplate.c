...
int net_put_ArrayTemplate(ArrayTemplate *arraytmp, NetBuf *net)
{
...

  {
    ...
    if(net_put_int(net, 1, &arraymap_revision))
      return 1;
  };
...
{
  ArrayTemplate *arraytmp; // The new template 
  ...
  {
    unsigned int arraymap_revision;
    if(net_get_int(net, 1, &arraymap_revision))
  ...
  for(regtemplate=0; regtemplate < arraytmp->ntemplate; regtemplate++) {
    ...
    if((regtemp->regtemplate=net_get_RegTemplate(net))==NULL)
      return del_ArrayTemplate(arraytmp);
  }
 ...
 */
int net_ArrayTemplate_size(ArrayTemplate *arraytmp)
{
  unsigned regtemplate; // The index of the register template being processed
  int size = 0;        // The byte-count to be returned 
  
#ifdef USE_REGMAP_REV
  size += arraytmp->ntemplate * NET_INT_SIZE;
#endif
 ...
 */
ArrayMap* net_get_ArrayMap(NetBuf *net)
{

 ...
 * The following define must be incremented by 1 whenever a
 * modification to the register map declarations makes
 * the register template incompatible with older archived register
 * maps. This number is recorded with archived register maps for
 * subsequent comparison against the current value.
 */
#define REGMAP_REVISION 1
...
enum RegFlags {
  ...
  REG_FAST   =  0x200000,// Include for multidimensional registers
			 // where the fastest-changing axis is a time
			 // subsample
};
 ...
 */
typedef enum {
  CBI_REG_DEFAULT = 0,    /* None of the following */
  CBI_REG_COMPLEX = 1,    /* Include if register contains real,imaginary pairs */
                      /* Exclude if register contains only real values */
  CBI_REG_SIGNED = 2,     /* Include if register contains signed values (2s-comp) *\/
			 /* Exclude if register contains unsigned values */
  CBI_REG_R_VME = 4,      /* Include if the register's vme address is readable */
                      /* Exclude to read written values from the shadow array *\/
			 REG_W_VME = 8,      /* Include if the register's vme address is writable */
                      /* Exclude if the register can't be written to */
  CBI_REG_W_VME = 8,      /* Include if the register's vme address is writable */
                      /* Exclude if the register can't be written to */
  CBI_REG_RW_VME = CBI_REG_R_VME | CBI_REG_W_VME,

  CBI_REG_SUM = 16,       /* Include for integrated registers */
                      /* Exclude if the register contains a snapshot value */
  CBI_REG_UNION = 32,     /* Including this tells the archiver to integrate */
                      /*  this register by taking the bit-mask union of */
                      /*  the values of the register that are to be combined. *\/
			  REG_EXC = 64,       /* Include to exclude the register from the archive */
                      /* Exclude to include the register in the archive */
  CBI_REG_EXC = 64,       /* Include to exclude the register from the archive */
                      /* Exclude to include the register in the archive */
  CBI_REG_UTC = 128       /* Include for registers that represent times in UTC. */
                      /*  Such registers must consist of one or more pairs */
                      /*  of elements in which the first element contains */
                      /*  a modified Julian day number, and the second */
                      /*  element contains the corresponding time of day in */
                      /*  milliseconds. */
} CbiRegFlags;
...
struct RegMapBlock {
  ...
  std::string* comment_;      // Description of register
  char axesHelp_[REG_NAME_LEN+1];  // Description of axis indices
  ...
  int iSlot_;                 // The sequential slot index of the
			      // first register in parent slot array
  int iArcSlot_;              // The sequential slot index of the
  ...
  // Return the number of butes per element of this register
  ...
  int byteOffsetInWholeRegMapOf(gcp::util::Coord* coord=0);
  int byteOffsetInArcRegMapOf(gcp::util::Coord* coord=0);

  int byteOffsetInBoardOf(bool archivedOnly, gcp::util::Coord* coord=0);

  // Convenience method for the above

  int byteOffsetInRegMapOf(bool archivedOnly, gcp::util::Coord* coord=0) {
  ...
  int slotOffsetInArcRegMapOf(gcp::util::Coord* coord=0) {
    return (iArcSlot_ < 0) ? -1 : (int)(iArcSlot_ + elementOffsetOf(coord));
  }

  int slotOffsetInWholeRegMapOf(gcp::util::Coord* coord=0) {
    return iSlot_ + (int)elementOffsetOf(coord);
  }

  // Return the start index of this block in the archived slot array

  int slotOffsetInRegMapOf(bool archivedOnly, gcp::util::Coord* coord=0) {
  ...
  int elementOffsetOf(gcp::util::Coord* coord);
...
#if USE_HASH
  HashTable *hash;           /* Symbol table of register blocks */
#else
  std::map<std::string, RegMapBlock*> mapOfBlocks_;
#endif
...
struct RegMap {
...
#if USE_HASH
  HashMemory *hash_mem_;     // Memory for hash tables 
  HashTable *hash_;          // Symbol table of boards 
#else
  std::map<std::string, RegMapBoard*> mapOfBoards_;
#endif
  ...
  RegMap(void *regtmp, bool addDefaults=true);
  ...
  int byteOffsetInArcRegMapOf(RegMapBlock* blk, gcp::util::Coord* coord=0);
  int byteOffsetInWholeRegMapOf(RegMapBlock* blk, gcp::util::Coord* coord=0);
...
enum {REG_NASPECT = 7};  /* The number of enumerators in RegAspect */
...
enum {REG_NINTEG = 3};  /* The number of enumerators in RegInteg */
...
struct RegMapReg {
  ...
  gcp::util::CoordAxes* axes_;   // An axis specifier for this register
  gcp::util::CoordRange* range_; // An index-range specifier
};

...
{
  RegSet* regset=0;             /* The container to be returned */
  unsigned i;
 ...
 */
int addRegSetRange(RegSet* regset, gcp::util::RegDescription& desc)
{
  try {
    std::vector<gcp::util::Range<unsigned> > ranges;

    ranges = desc.getByteRanges();
    for(unsigned iRange=0; iRange < ranges.size(); iRange++) {
      DBPRINT(true, Debug::DEBUG2, "Range is: " << ranges[iRange]);
 ...
 */
int net_RegSet_size(RegSet *regset)
{
  return NET_INT_SIZE +                    /* The revision count */
...
{
  unsigned int tmp[3];   /* Used to send data to net_put_short() */
 ...
 */
  {
    unsigned int revision = ++regset->revision;
    if(net_put_int(net, 1, &revision))
      return 1;
  };
/*
...
{
  unsigned int ltmp[3];   /* Used to send data to net_put_short() */
  RegSetRange *range;       /* The range being packed */
/*
 * Check arguments.
 */
...
{
  unsigned int tmp[3];   /* Used to send data to net_get_short() */
 ...
 * to the list.
 */
    if(net_get_int(net, 2, tmp) ||
       (last=insert_RegSetRange(regset, last, tmp[0], tmp[1])) == NULL)
      return ngRS_error(regset);

    DBPRINT(true, Debug::DEBUG3, "Got a new range: " 
	    << " start = " << tmp[0]
	    << " stop  = " << tmp[1]);
  };
  return 0;
...
{
  unsigned int ltmp[3];   /* Used to send data to net_get_short() */
  RegSetRange *last;        /* The last range added to the register set */
  int nrange;               /* The number of ranges to be read */
  int i;
/*
 ...
 * to the list.
 */
    if(net_get_int(net, 2, ltmp) ||
       (last=insert_RegSetRange(regset, last, ltmp[0], ltmp[1])) == NULL)
      return ngRS_error(regset);

    DBPRINT(false, Debug::DEBUG10, "Got a new range: " 
	    << " start = " << ltmp[0]
	    << " stop  = " << ltmp[1]);
  };
  return 0;
 ...
 *                     set of register ranges.
 */
int net_regs_size(RegSet *regset)
{
  return NET_INT_SIZE +           /* The regset revision count */
    NET_SHORT_SIZE * 1 +           /* The count of selected elements */
    NET_INT_SIZE * regset->nslot; /* The register-slot values */
}
...
int net_put_regs(RegSet *regset, RegRawData *raw, NetBuf *net)
{
  ...
  for(range = regset->ranges; range; range=range->next) {
    ...
    if(net_put_int(net, range->ib - range->ia + 1, 
		    (unsigned int*)(raw->slots + range->ia)))
      return 1;
  };
  return 0;
}
...
NetRegState netGetRegs(RegSet* regset, RegRawData* raw, NetBuf *net)
{
  ...
  DBPRINT(false, Debug::DEBUG3, "RegSet is: " << regset->nByte_ << " bytes long");
  DBPRINT(false, Debug::DEBUG3, "frame is: "  << raw->fm->sizeInBytesOfData() << " bytes long");
...
NetRegState net_get_regs(RegSet *regset, RegRawData *raw, NetBuf *net)
{
  ...
  for(range = regset->ranges; range; range=range->next) {
    if(net_get_int(net, range->ib - range->ia + 1, 
		    (unsigned int*)(raw->slots + range->ia)))
      return NETREG_ERROR;
  };
  return NETREG_OK;
...
int equiv_RegSet(RegSet *regset1, RegSet *regset2)
{
  ...
  DBPRINT(true, gcp::util::Debug::DEBUG3, "regset1 has: "
	  << regset1->nByteSelected
	  << " bytes selected");

  DBPRINT(true, gcp::util::Debug::DEBUG3, "regset2 has: "
	  << regset2->nByteSelected
	  << " bytes selected");

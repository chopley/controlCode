...
{
  InputSource *source=0;   /* The container of the new input-source specifics */
/*
 * Check arguments.
 */
...
{
  FileInput *fdata=0; /* The file-specific context to be registered */
/*
...
int read_InputStream(InputStream *stream, int tell)
{
  ...
  // Preserve the previous value of stream->was_escape before
  ...
  // If the last returned character was an escape character then the
  // next character has already been read.
  ...
  lprintf(stdout,"Last returned character was escaped\n");
    ...
    // If we have reached the end of the current input source, close
    // it and continue the read from its parent (if it has one).
  ...
  case '\\':         /* Check for escaped newlines and escaped quotes */
	...
	return 1;
      ...
      // If the escaped character is a newline character then skip
      // both the escape and the newline and return the next character
      // after the newline.
	...
	// Record the escaped character so that it can be returned on
	// the next call to this function, and record the escape
	// character for return this time.
  ...
  // Copy the newly read character into the stream. At the end of the
  // input source add a newline. This ensures that the last line of
  // the source is properly terminated and because it has the side
  // effect of deferring the closure of the stream until the next call
  // to this function, it allows errors pertaining to the last
  // character of the source to be prefixed by input_error() using the
  // error-prefix function of the current source.
...
int input_board_regexp_keyword(InputStream *stream, int tell, int fold)
{
  int i;
/*
 * Copy up to INPUT_WORKLEN-1 alphanumeric and underscore characters
 ...
 *                        else decimal.
 * Input/Output:
 *  lval          int *  On output *lval will contain the
 *                        int integer read from the input stream.
 * Output:
 *  return         int    0 - OK.
 *                        1 - Error.
 */
int input_int(InputStream *stream, int tell, int anybase, int *lval)
{
  unsigned int ulval;  /* The return value of input_uint() */
...
/*
 * Having removed the sign prefix, get input_uint() to read the unsigned
 * part.
 */
  if(input_uint(stream, 0, anybase, &ulval))
...
/*.......................................................................
 * Read an unsigned int integer from an input stream.
 *
 * Input:
 *  stream   InputStream *  The stream to read the integer from.
 *  tell             int    Non-zero to report errors to stderr.
 ...
 * Input/Output:
 *  ulval  unsigned int *  On output *ulval will contain the unsigned
 *                          int integer read from the input stream.
 * Output:
 *  return           int    0 - OK.
 *                          1 - Error.
 */
int input_uint(InputStream *stream, int tell, int anybase, unsigned int *ulval)
{
...
int input_date(InputStream *stream, int tell, int *year, int *month, int *day)
{
  ...
  unsigned int dy;         /* The day of the month */
  unsigned int mn;         /* The month of the year */
  unsigned int yr;         /* The year */
 ...
 */
  if(dy < 1 || dy > (unsigned int)(daytab[isleap][mn-1])) {
...
int input_time(InputStream *stream, int tell, int *hour, int *min, double *sec)
{
  ...
  unsigned int v;    /* An hour or minute value */
 ...
 */
  if(input_uint(stream, 0, 0, &v) || v > 23)
    return input_error(stream, tell, usage);
  *hour = v;
/*
 ...
 */
    if(read_InputStream(stream, 0) || input_uint(stream, 0, 0, &v) || v > 59)
      return input_error(stream, tell, usage);
    *min = v;
  } else {
    *min = 0;

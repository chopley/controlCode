#define __FILEPATH__ "control/code/unix/libunix_src/common/regmap.c"
 ...
 */
RegMap::RegMap(void* vtmp, bool addDefaults)
{
  int i;
  ...
  static RegBlockTemp generic_regmap_board[] = {
    ...
    // This register is set to gcp::util::FrameFlags::RECEIVED if this
    // frame has been received from a remote computer
    
    RegBlockTemp("Bit 1 will be high if data in this frame were received during"
		 "the last integration.  If not, bit 0 will be set high", 
		 "received",   "", REG_UCHAR|REG_UNION, 0, 1),
    
    // The number of samples per integration 
    
    RegBlockTemp("The number of frames which have been integrated to produce"
		 "this frame", 
		 "nsnap",      "", REG_UINT|REG_SUM, 0, 1),

    // The sequential record number 
    
    RegBlockTemp("A monotonically-increasing record number since the last "
		 "re-start of the control system", 
...
RegMapBoard *find_RegMapBoard(RegMap *regmap, string board_name)
{
  ...
  std::map<std::string, RegMapBoard*>::iterator slot;

  slot = regmap->mapOfBoards_.find(board_name);

  if(slot == regmap->mapOfBoards_.end()) 
    return NULL;
  else {
    return slot->second;
  }
...
{
  std::map<std::string, RegMapBlock*>::iterator slot;

  slot = mapOfBlocks_.find(blockName);

  if(slot == mapOfBlocks_.end()) 
    return NULL;
  else {
    return slot->second;
  }
...
{
  if(!board) {
    lprintf(stderr, "find_RegMapBoard_Block: NULL board.\n");
    return NULL;
  };

  std::map<std::string, RegMapBlock*>::iterator slot;

  slot = board->mapOfBlocks_.find(block_name);

  if(slot == board->mapOfBlocks_.end()) 
    return NULL;
  else {
    return slot->second;
  }
...
RegMapBlock *find_RegMapBlock(RegMap *regmap, string board_name,
			      string block_name)
{
  RegMapBoard* board=0;   /* The named board */
			    ...
			    RegInputMode mode, int extend, RegMapReg *reg)
{
  ...
  int index2;                   /* The highest block index (-1 for max) */
  ...
  if(reg) {
    reg->board = regmap ? (int)board->number : -1;
    reg->block = regmap ? (int)block->number_ : -1;
    reg->index = index1;
...
int unpack_double_string(double *data, int ndata, int size, char *string)
{
  ...
  for(i=0; i<ndata; i++) {
     ...
     */
    unsigned int u = (d < 0 || d > UINT_MAX) ? 0 : (unsigned int)d;
    /*
			 ...
			 unsigned iBlock, unsigned nper_brd_blocks)
{
  ...
  strcpy(axesHelp_, tblk->axesHelp_);
  ...
  parent->mapOfBlocks_[name_] = this;
  ...
  default:
    {
      ThrowError("Unknown address mode for " 
		 << parent->name << "." << name_);
    }
    break;
  };
 ...
 * register in the parent board
 */
int RegMapBlock::byteOffsetInBoardOf(bool archivedOnly, gcp::util::Coord* coord)
{
  return byteOffsetInRegMapOf(archivedOnly, coord) - 
    brd_->byteOffsetInRegMap(archivedOnly);
}
 ...
 * register in the register map archive array
 */
int RegMapBlock::elementOffsetOf(gcp::util::Coord* coord)
{
			 ...
			 void* vper_brd_blocks, unsigned nper_brd_blocks)
{
  ...
  // All boards must have unambiguous names.
  
  regmap->mapOfBoards_[name] = this;

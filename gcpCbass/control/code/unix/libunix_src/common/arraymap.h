 ...
 * modification to the array map declarations makes
 * the register template incompatible with older archived array
 * maps. This number is recorded with archived array maps for
 * subsequent comparison against the current value.
 */
#define ARRAYMAP_REVISION 1
 ...
 */
struct ArrRegMapReg {
  int regmap; // The index of the specified regmap in the array map
  RegMapReg reg;
};
...
struct ArrRegMap {
  ...
  int byteOffsetInArrayMapOf(bool archivedOnly, 
			     RegMapBlock* block, gcp::util::Coord* coord=0) 
  {
...
struct ArrayMap {
...
#if USE_HASH
  HashTable *hash;           /* Symbol table of register maps */
  HashMemory *hash_mem;      /* Memory for hash tables */
#else
  std::map<std::string, ArrRegMap*> mapOfRegmaps_;
#endif
  ...
  RegMapBlock* findArrayMapBlock(std::string regmap_name, std::string board_name, 
				 ...
				 bool doThrow=false);
  // Find the named board

  RegMapBoard* findArrayMapBoard(std::string regmap_name,

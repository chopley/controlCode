...
#include "util/common/Exception.h"
...
static OPER_FN(sc_add_sexagesimal);
static OPER_FN(sc_sub_sexagesimal);
...
typedef struct {
  ...
  ScOperator *add_op;         /* The addition operator */
  ScOperator *sub_op;         /* The subtraction operator */
  ScOperator *mult_op;        /* The multiplication operator */
  DataType   *add_dt;         /* The Double datatype used in addition */
  DataType   *mult_dt;        /* The datatype used in multiplication */
} SexagesimalContext;
...
DataType *add_ChoiceDataType(Script *sc, char *name,
			     Enumerator *choices, int nchoice)
{
 ...
 */
  for(i=0; i < (int)ct->nchoice; i++) {
    if(strcmp(stream->work, ct->choices[i].name) == 0)
      break;
  };
  if(i >= (int)ct->nchoice) {
...
static DT_CONST(const_set)
{
  SetType *st;          /* The set specific datatype members */
  Variable *var;        /* A set-constant */
  unsigned int value;  /* The value of the constant */
  int i;
/*
 * Get the set specific members of the data-type.
 */
 ...
 */
  if(st->allow_bit_mask && isdigit(stream->nextc)) {
    if(input_uint(stream, 0, 1, &value))
 ...
 */
    for(i=0; i < (int)st->nmember; i++) {
      if(strcmp(stream->work, st->members[i].name) == 0)
	break;
    };
    if(i >= (int)st->nmember) {
...
static DT_CONST(sc_const_uint)
{
  ...
  unsigned int ul;     /* The literal value */
/*
 * Read the number from the input stream.
 */
  if(input_uint(stream, 0, 1, &ul))
...
static DT_PRINT(sc_print_uint)
{
  return output_uint(output, OUT_DECIMAL, "", 0, 0,
...
{
  Variable *var;        /* The variable that will contain the number */
  int sl;              /* The literal value */
  
  // Read the number from the input stream.

  if(input_int(stream, 0, 1, &sl))
    return input_error(stream, 1, "Missing integer '%s' value.\n", dt->name);
  
  // Record the number in a new variable and push the variable onto
...
static DT_PRINT(sc_print_int)
{
  return output_int(output, OUT_DECIMAL, "", 0, 0,
		     INT_VARIABLE(var)->i);
}
...
/*.......................................................................
 * Define the sexagesimal multiplication operator function. This is a binary operator
 * that expects a sexagesimal value on its left and a an integer value on its
 * right. The result is a new sexagesimal.
 */
static OPER_FN(sc_mult_sexagesimal)
{
  COUT("Inside mult_sexagesimal");
  ...
  // Multiply the doubles, and record the result for return.

  DOUBLE_VARIABLE(result)->d = DOUBLE_VARIABLE(v1)->d * UINT_VARIABLE(v2)->uint;
				  ...
				  int allow_negation)
{
  ...
  context->add_op  = NULL;
  context->sub_op  = NULL;
  context->mult_op = NULL;

  context->add_dt  = NULL;
  context->mult_dt = NULL;
  ...
  context->mult_dt = find_DataType(sc, NULL, "Integer");
  if(!context->mult_dt) {
    lprintf(stderr, "new_DataType(%s): Can't find the Integer datatype.\n",
	    name);
    return NULL;
  ...
  context->add_op = new_ScOperator(sc, dt, 2, sc_add_sexagesimal);
  if(!context->add_op)
    return NULL;

  context->sub_op = new_ScOperator(sc, dt, 2, sc_sub_sexagesimal);
  if(!context->sub_op)
    return NULL;

  context->mult_op = new_ScOperator(sc, dt, 2, sc_mult_sexagesimal);
  if(!context->mult_op)
    return NULL;
...
static DT_PARSE(sc_parse_sexagesimal)
{
  ...
  while(1) {
	...
	return 1;
      ...
      // Read the following operand.
      
      if(parse_operand(sc, s->mult_dt, 0, stream, e))
	return 1;
      ...
      if(!add_OpFnOper(sc, e, s->mult_op))
	return 1;
      break;
    default:
      return 0;   /* End of expression */
...
//-----------------------------------------------------------------------
// A Sexagesimal type wrapped at 24
//-----------------------------------------------------------------------

static OPER_FN(sc_add_sexagesimal24);
static OPER_FN(sc_sub_sexagesimal24);
...
static OPER_FN(sc_add_sexagesimal24)
{
  ...
  double d = DOUBLE_VARIABLE(v1)->d + DOUBLE_VARIABLE(v2)->d;
 ...
 *  return    DataType *   The new datatype.
 */
DataType *add_Sexagesimal24DataType(Script *sc, char *name, DT_CHECK(*check_fn),
				    DT_ITER(*iter_fn), char *incr_name,
				  int allow_negation)
{
  DataType *dt;                 /* The object to be returned */
  ...
  context = (SexagesimalContext* )new_ScriptObject(sc, NULL, 
						   sizeof(SexagesimalContext));
  if(!context)
    return NULL;
  ...
  context->add_op = new_ScOperator(sc, dt, 2, sc_add_sexagesimal24);
  if(!context->add_op)
    return NULL;

  context->sub_op = new_ScOperator(sc, dt, 2, sc_sub_sexagesimal24);
  if(!context->sub_op)
    return NULL;

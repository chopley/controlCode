 ...
 */
  if(dt) {
    if(type->dt->id != DT_UNK && dt->id != DT_UNK && type->dt->id == dt->id)
      return type;

    if(type->dt != dt || !type->is_list != !is_list) { 
      input_error(stream, 1, "The entered $ expression doesn't yield a value of the required datatype %s.\n", stream->work);
      return NULL;
    };

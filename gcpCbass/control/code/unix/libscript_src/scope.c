 ...
 * delete it.
 */
  del_HashTable((HashTable *)del_ListNode(sc->script.scopes, 
					  sc->script.scopes->head, NULL));
  return 0;
}
 ...
 */
Symbol *add_ScriptSymbol(Script *sc, char *name, SymbolType code, 
...
Symbol *add_ScriptSymbol(Script *sc, char *name, SymbolType code, void *data, bool isAlias)
{
  HashTable *hash;   /* The symbol table to add the symbol to */
/*
 * Find the appropriate hash table.
 */
  if(sc->script.state == SCRIPT_EMPTY || isAlias) {
 ...
 */
Symbol *find_ScriptSymbol(Script *sc, InputStream *stream, char *name, bool printError)
{

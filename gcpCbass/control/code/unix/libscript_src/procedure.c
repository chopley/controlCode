...
int add_UserAlias(Script *sc, char* command, char* alias)
{
  Symbol* symbol=0;
  ...
  if(!sc || !command || !alias) {
    lprintf(stderr, "add_UserAlias: NULL argument(s).\n");
    return 1;
  };

  // First make sure we don't already have a symbol by that name
  
  symbol = find_ScriptSymbol(sc, NULL, alias, false);

  if(symbol) {
    lprintf(stderr, "add_UserAlias: Symbol name: %s already exists\n", alias);
    return 1;
  }

  symbol = find_ScriptSymbol(sc, NULL, command);

  if(!symbol) {
    lprintf(stderr, "add_UserAlias: Symbol name: %s not found\n", command);
    return 1;
  }

  // Now that we have the symbol, add a new symbol with the alias name

  if(!add_ScriptSymbol(sc, alias, (SymbolType)symbol->code, symbol->data, true)) {
    lprintf(stderr, "add_UserAlias: Unable to add alias: %s\n", alias);
    return 1;
  }

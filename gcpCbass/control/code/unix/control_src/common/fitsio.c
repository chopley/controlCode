...
static int mjd_to_date(unsigned int mjdday, unsigned int mjdms, char *date);
 ...
 * header keyword.
 */
int mjd_to_date(unsigned int mjdday, unsigned int mjdms, char *date)
{
 ...
 *                     1 - Error.
 */	       
int net_put_fitshead(NetBuf *net, unsigned int utc[2])
{ 
  ...
  os << "Written by " << gcp::util::SpecificName::experimentName() << "Control";
  status |= put_phdu(buf, (unsigned char*)"SIMPLE", val, 
		     (unsigned char*)os.str().c_str());
   ...
   */
  strcpy((char*)val.val_c, gcp::util::SpecificName::experimentName().c_str());

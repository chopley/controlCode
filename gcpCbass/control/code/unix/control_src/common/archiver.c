...
#include "ArchiverWriterFrame.h"
#include "ArchiverWriterDirfile.h"
...
static int arc_update_sampling(Archiver *arc, unsigned nframe);
...
CP_NEW_FN(new_Archiver)
{
  ...
  arc->fb.start_ok = 1;
  /*
   * Allocate the frame-store in which integrated frames are accumulated.
   */
   ...
   */
  {
    ...
    int size3 = net_RegRawData_size(arc->fb.frame); /* Frame record */
   ...
   */
  if(!(arc->fb.features = arc_find_reg(arc, "array", "frame", "features")) ||
     !(arc->fb.markSeq  = arc_find_reg(arc, "array", "frame", "markSeq")))
    return del_Archiver(arc);
  
  // Allocate the writer
  
  if(cp_dirfileArchive(cp))
    arc->writer_ = new gcp::control::ArchiverWriterDirfile(arc, 1);
  else
    arc->writer_ = new ArchiverWriterFrame(arc);
 ...
 * Output:
 *  return unsigned int *   The address of the register in fb->frame->slots[].
 */
unsigned int* arc_find_reg(Archiver *arc, char* regmap, char *board, 
			   char *name)
{
  gcp::util::ArrayDataFrameManager* fm = arc->fb.frame->fm;
  ...
  return fm->frame()->getUintPtr(offset);
}
...
{
  gcp::util::ArrayDataFrameManager* fm = arc->fb.frame->fm;
  int offset = fm->byteOffsetInFrameOf(regmap, "tracker", "actual");
  
  if(offset < 0) {
    lprintf(stderr, "new_Archiver: Lookup of %s.tracker.actual failed.\n", regmap);
    return NULL;
  }
  
  return fm->frame()->getDoublePtr(offset);
}
...
int arc_integrate_frame_old(ControlProg *cp, RegRawData *frame)
{
  ...
  if(fb->count == 0) {
    ...
    int *from = frame->slots;
    int *to   = fb->frame->slots;
  ...
  DBPRINT(false, Debug::DEBUG10, "array.frame.mjd is: " <<
	  *arc_find_reg(arc, "array", "frame", "utc"));
...
CP_THREAD_FN(archiver_thread)
{
  ...
  while(select(fdSet.size(), fdSet.readFdSet(), NULL, NULL, NULL) > 0) {
    ...
    case ARC_CHDIR:
      ...
      break;
    case ARC_OPEN:
      (void) arc->writer_->openArcfile(msg.body.open.dir);
      break;
    case ARC_FLUSH:
      flush_arcfile(arc);
      break;
    case ARC_CLOSE:
      ...
      break;
    case ARC_SAMPLING:
      (void) arc_update_sampling(arc, msg.body.sampling.nframe);
      break;
    case ARC_FILTER:
...
int open_arcfile(Archiver *arc, char *dir)
{
  ...
  if(net_start_put(net, ARC_SIZE_RECORD) ||
     net_put_int(net, 1, &size) ||
     net_end_put(net) ||
     (int)fwrite(net->buf, sizeof(net->buf[0]),
...
static int arc_save_integration(Archiver *arc)
{
  ...
  /*
   * Construct an output record of the integrated data.
  ...
  fb->count = 0;
  /*
   * Signal other threads that the frame-buffer is now ready for a
   * new integration.
   */
  ...
  pthread_mutex_unlock(&fb->guard);
  /*
   * Having safely released exclusive access to the frame buffer, abort if
   * there was an error.
   */
  if(status)
    return 1;
  /*
   * If the effects of a new mark command are being saved, send a message
   * to the scheduler to tell it that the mark transaction has been
   * completed.
   */
  ...
  if(arc->writer_->isOpen() && (!arc->filter || *fb->features)) {
    
    last.setToCurrentTime();
    
    if(arc->writer_->writeIntegration()) {
      arc->writer_->closeArcfile();
      return 1;
    };
    ...
    /*
     * If the current archive file has reached the maximum size specified by
     * the user, open a new one for the next record.
     */
    if(arc->file_size > 0 && ++arc->nrecorded >= arc->file_size)
      return arc->writer_->openArcfile(arc->dir);
    /*
 ...
 *                        1 - Error.
 */
int arc_send_ccmsg(Archiver *arc, Pipe *client, const char *fmt, ...)
{
  ...
  if(nsent < 0)
    return 1;
  /*
   * Send the message to the specified client, or all connected clients.
   */
  if(client) {
		 ...
		 short index, unsigned int *val)
{
  ...
  *val = *(arc->fb.frame->slots + blk->slot_ + index);

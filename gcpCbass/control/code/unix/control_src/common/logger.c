...
struct Logger {
  ...
  // An object to manage log message buffering
...
static int log_add_client(Logger *log, gcp::util::Pipe *client);
static int log_rem_client(Logger *log, gcp::util::Pipe *client);
...
CP_NEW_FN(new_Logger)
{
  ...
  // Allocate a new log msg handler
  
  log->logMsgHandler_ = new gcp::util::LogMsgHandler();
...
CP_DEL_FN(del_Logger)
{
  ...
  if(log) {
    ...
    // Delete the transaction manager
    ...
    if(log->logMsgHandler_ != 0)
      delete log->logMsgHandler_;
...
static int write_logfile(Logger *log, char *text, LogStream nature)
{
  ...
  /*
   * If the error indicator of the stream is set, clear it and attempt to
   * start a newline, to see if the causative condition (such as a full
   * disk) has been cleared. If this fails, attempt to log the message to
   * the controlling terminal of the program.
   */
  if(ferror(log->fp)) {
    clearerr(log->fp);
...
static int write_terminal(Logger *log, char *text, LogStream nature)
{
  ...
  if(fputs(text, fp) == EOF ||
     fputc('\n', fp) == EOF) {
    lprintf(stderr, "Error writing to %s of genericcontrol.\n",
...
int log_thread_stream(ControlProg *cp, FILE *stream)
{
  ...
  int waserr = divert_lprintf(stream, lprintf_to_logger, cp, NULL, NULL);

  try {
    if(stream == stdout)
      gcp::util::Logger::installLogHandler(stdoutLogger);
    else
      gcp::util::Logger::installErrHandler(stderrLogger);
  } catch(...) {
    return 1;
  }
...
LOG_DISPATCHER(lprintf_to_logger)
{
  ControlProg *cp = (ControlProg* )context;   /* The resource object
						 of the control
						 program */
  LoggerMessage msg;           /* The packed version of the message */
  /*
   * To avoid deadlocks between threads, don't use blocking I/O to write
   * the message. If the logger pipe fills up, we'll simply lose a
   * log message or two. If this happens frequently, then there is something
   * seriously wrong with the scheduling of the control program threads.
   */
  return pack_logger_message(&msg, message, id) ||
    send_LoggerMessage(cp, &msg, PIPE_NOWAIT) == PIPE_ERROR;
}
				...
				unsigned seq, bool end, bool interactive)
{
  ...
  int ntext;             /* The number of characters written to text[] */
  ...
  if(utc.year != log->year || utc.month != log->month || utc.day != log->day) {
    ...
    // If we are already logging open a new file in the current
    // directory
  ...
  log->logMsgHandler_->append(seq, text);
  ...
  if(end) {
    ...
    std::string completeMessage = 
      log->logMsgHandler_->readMessage(seq);
    
    do {
      
      nmsg=0;
      
      // Only prepend the timestamp prefix if this is the first
      // segment of the message
      
      if(first) {
	
	sprintf(log->msg, "%02d%02d%02d %02d:%02d:%02d %n",utc.year%100,
	...
	os << log->msg;
      ...
      // Get the next substring from the message handler
      
      message = 
	log->logMsgHandler_->
	getNextMessageSubstr(seq, 
			     (first ? LOG_MSGLEN-nmsg : LOG_MSGLEN), isLast);
      
      // Append the log message.
      
      strncpy(log->msg + nmsg, (char*) message.c_str(), message.size());
      nmsg += message.size();
      log->msg[nmsg] = '\0';
      
      // Dispatch the message to all connected log clients.
      
      (void) write_logclient(log, log->msg, nature, seq, isLast, interactive);
      
      first = false;
      
    } while(!isLast);
    
    // Write a copy of the message to the log file all at one go
    
    os << completeMessage;
    (void) write_logfile(log, (char*)os.str().c_str(), nature);
  }
...
static int log_date_message(Logger *log, Date *date)
{
   ...
   */
  mname = name_of_month(date->month, 0, 1);
  if(!mname)
    return 1;
  /*
   ...
   * Log the message.
   */
  return dispatch_log_message(log, date_message, LOG_STDOUT);
}
			...
			char* location, double date, char* who, char* comment)
{
  ...
  unsigned waserr;
  waserr =  dispatch_log_message(log, (char*)message.str().c_str(), LOG_STDOUT);
  ...
  if(comment[0] != '\0') {
    ...
    waserr |=  
      dispatch_log_message(log, (char*)message.str().c_str(), LOG_STDOUT);
  }
  return waserr;
}

...
struct PipeChan {
  ...
  gcp::util::Pipe* pipe;   /* The unnamed pipe */
...
typedef struct {
  ...
  int nss_size;     /* The size of the network send buffers */
  int nrs_size;     /* The size of the network read buffers */
...
typedef struct {
  int port;          /* The TCP/IP port on which to listen for clients */
  int nss_size;     /* The size of the network send buffers */
  int nrs_size;     /* The size of the network receive buffers */
...
static void addPagerRegister(ControlProg* cp, std::string regSpec,
			     ...
			     unsigned nFrame, bool outOfRange, std::string script);
 ...
 * enumerators defined in genericcontrol.h.
 */
typedef struct {
...
static CpThreadType thread_table[] = {
  {CP_SCHEDULER, new_Scheduler, del_Scheduler, scheduler_thread,
   stop_Scheduler, sent_Scheduler, SchedulerMessage::msgSize()},
...
namespace gcp {
  namespace control {
    ...
    public:
      ...
      gcp::util::TimeVal rtcCmdTimeOut_; // A timeout that will
                                         // indicate no commands have
                                         // been sent to the rtc

      gcp::util::TimeVal rtcDataTimeOut_;// A timeout that will
                                         // indicate no data have been
                                         // received by the rtc


      gcp::util::TimeVal lastCmdSent_;   // The last time a command
					 // was sent

      gcp::util::TimeVal lastDataRcvd_;  // The last time data were
					 // received

      bool pagingAllowed_;               // True if the pager can be
					 // activated
    ...
    TimeOut::TimeOut() {
      ...
      // True if we should reset the timeout on the current command
      // (some commands, like navigator updates, should not cause the
      // timeout to be reset)

      resetTimeOutOnCurrentCommand_ = true;

      // Set the actual timeout interval

      setIntervalInSeconds(5);

      // Set the command and data timeout intervals

      setCmdIntervalInSeconds(defaultInterval_);
      setDataIntervalInSeconds(defaultInterval_);

      // Leave the timeouts deactivated until the commands to activate
      // them are received

      activateCmdTimeout(false);
      activateDataTimeout(false);

      // Initially, we assume that paging is allowed

      allowPaging(true);

      // Stage a reset of the data and command clocks on the next
      // entry into select()

      cmdResetPending_  = true;
      dataResetPending_ = true;
    }
    ...
    void TimeOut::setCmdIntervalInSeconds(unsigned int seconds) {
      rtcCmdTimeOut_.setTime(seconds,0);
      cmdResetPending_ = true;
    }

    void TimeOut::setDataIntervalInSeconds(unsigned int seconds) {
      rtcDataTimeOut_.setTime(seconds,0);
      dataResetPending_ = true;
    }
    ...
    void TimeOut::activateCmdTimeout(bool active) {
      cmdTimeoutActive_ = active;
    }

    //------------------------------------------------------------
    // Activate the data timeout
    //------------------------------------------------------------

    void TimeOut::activateDataTimeout(bool active) {
      dataTimeoutActive_ = active;
    }

    //------------------------------------------------------------
    // Return a pointer to our timeout object suitable for use
    // in select()
    //------------------------------------------------------------

    struct timeval* TimeOut::tVal() {

      // If the command pending flag was set, reset the timeout now

      if(cmdResetPending_) {
	lastCmdSent_.setToCurrentTime();
	cmdResetPending_ = false;
      }

      if(dataResetPending_) {
	lastDataRcvd_.setToCurrentTime();
	dataResetPending_ = false;
      }
    ...
    //------------------------------------------------------------
    // Set whether or not paging is allowed.  Called when the pager is
    // re-enabled, so we should reset the deadman counters as well
    //------------------------------------------------------------
    ...
    //------------------------------------------------------------
    // Register that a command timeout has occurred
    //------------------------------------------------------------
    ...
    int TimeOut::registerTimeOut(ControlProg* cp) {
      ...
      TimeVal currTime, diff;
      currTime.setToCurrentTime();

      //------------------------------------------------------------
      // Check if we should activate the pager on command timeout

      if(pagingAllowed() && cmdTimeoutActive_) {

	diff = currTime - lastCmdSent_;
	
	if(diff.getTimeInSeconds() > rtcCmdTimeOut_.getTimeInSeconds()) {
	  ...
	  if(pack_term_msg_page(&msg, "No commands were sent to the RTC") ||
      ...
      //------------------------------------------------------------
      // Check if we should activate the pager on data timeout

      if(pagingAllowed() && dataTimeoutActive_) {

	diff = currTime - lastDataRcvd_;

	if(diff.getTimeInSeconds() > rtcDataTimeOut_.getTimeInSeconds()) {
	  ...
	  if(pack_term_msg_page(&msg, "No data were received by the RTC") ||
	     send_TermMessage(cp, &msg, PIPE_WAIT)==PIPE_ERROR)
	    return 1;
	}
      }
    ...
    void TimeOut::resetDataTimeout() {
      lastDataRcvd_.setToCurrentTime();
    }

    //------------------------------------------------------------
    // Register that the command timeout should be deactivated
    // the next time a command is sent
    //------------------------------------------------------------

    void TimeOut::registerDeactivateCmdTimeout(bool deactivate) {
 ...
 */
struct ControlProg {
  ArrayMap *arraymap;       /* The register map */
  ...
  gcp::util::AntNum defaultAntSet;     // A set of antennas to be used
				       // as the default for command
				       // which take an optional
				       // antenna argument

  // An object for managing timeouts when communicating with the rtc

  gcp::control::TimeOut* timeOut_;

  std::string* startupScript_;
  bool dirfileArchive_;
  bool useModemPager_;
};

static ControlProg *new_ControlProg(std::string startupScript, 
				    bool dirfileArchive, bool useModemPager);
...
typedef enum {
  ...
  CP_ADD_PAGER_REG_MSG, // Add a register to the list to be monitored
                        // for paging
  CP_REM_PAGER_REG_MSG, // Remove a register from the list to be
                        // monitored for paging
...
KeyTabEntry Program::keywords[] = {
  { "startupScript",  "",       "s",  "Startup script"},
  { "useModemPager",  "t",      "b",  "If true, modem paging will be allowed"},
  { "dirfileArchive", "f",      "b",  "Archive data in dirfile format?"},

  { END_OF_KEYWORDS,END_OF_KEYWORDS,END_OF_KEYWORDS,END_OF_KEYWORDS},
};

static int processGenericMsg(ControlProg* cp, SockChan* sock, 
			     gcp::util::NewRtcNetMsg& netmsg);

/*.......................................................................
 * This is the entry point of the control program.
 *
 * Input:
 *  argc     int     The number of elements in argv[].
 *  argv    char *[] Command-line arguments:
 *                   [0] - The name of the program.
 */
int Program::main()
{
  ControlProg *cp;                  /* The program resources */
  int waserr = 0;                   /* True after errors */
  CpWhatNext whatnext = CP_RESTART; /* The continuation state of the program */

  Debug::setLevel(Program::getiParameter("debuglevel"));

  try {
  // Allow the control program to be restarted.

  while(whatnext == CP_RESTART && !waserr) {
    
    // Allocate the resources of the control program and create its
    // threads.

    if(Program::isDefault("startupScript")) {
      cp = new_ControlProg(gcp::control::InitScript::initScript().c_str(), getbParameter("dirfileArchive"), getbParameter("useModemPager"));
    } else {
      cp = new_ControlProg(getParameter("startupScript"), getbParameter("dirfileArchive"), getbParameter("useModemPager"));
    }
    ...
    waserr = cp_event_loop(cp);
    
    // Record the continuation status before deleting the object that
    // contains it!

    whatnext = cp->whatnext;
    
    // Terminate the other threads and reclaim resources.
				    ...
				    bool useModemPager)
{
  ...
  cp->dirfileArchive_  = dirfileArchive;
  cp->useModemPager_   = useModemPager;
  ...
  cp->defaultAntSet.set(AntNum::ANTALL);

  // Initialize the timeout

  cp->timeOut_ = new gcp::control::TimeOut();
 ...
 *  return   ControlProg *  The resource object, or NULL on error.
 */
ControlProg* new_ControlProg(bool createScheduler, bool createNavigator)
{
  ControlProg *cp;    /* The resource object to be returned */
  int status;         /* The return status of a pthreads function */
  int i;
  ...
  cp->dirfileArchive_  = false;
  cp->useModemPager_   = false;
  ...
  // Create the scheduler thread resources if requested

  if(createScheduler)  {
    unsigned i = CP_SCHEDULER;
    CpThread *t = cp->thread + i;
    CpThreadType *tt = t->type;
    t->pipe = new_PipeChan(cp, t, tt->sizeof_message, 0, tt->sent_fn);
    if(!t->pipe)
  ...
  // Create the navigator thread resources if requested

  if(createNavigator)  {
    unsigned i = CP_NAVIGATOR;
    CpThread *t = cp->thread + i;
    CpThreadType *tt = t->type;
    t->pipe = new_PipeChan(cp, t, tt->sizeof_message, 0, tt->sent_fn);
    if(!t->pipe)
...
ControlProg *del_ControlProg(ControlProg *cp)
{
  ...
  if(cp) {
    ...
    cp->arraymap = del_ArrayMap(cp->arraymap);
    free(cp);
  };
...
/*.......................................................................
 * Provide readonly access to the register map to all threads.
 *
 * Input:
 *  cp      ControlProg *  The resource object of the control program.
 * Output:
 *  return       RegMap *  The register map.
 */
...
static int cp_event_loop(ControlProg *cp)
{
  ...
  while(!waserr && cp->whatnext==CP_CONTINUE) {
    ...
    //    COUT("fd_set_size before select = "  << cp->fd_set_size); 
    //      for(chan=cp->active_list.head; chan; chan = chan->next) {
    //	COUT("chan->fd = " << chan->fd);
    //      };
    nready = select(cp->fd_set_size, &read_fds, &send_fds, NULL, 
		    cp->timeOut_->tVal());
    //    COUT("fd_set_size after select = "  << cp->fd_set_size); 
    ...
    default:           /* One or more fds are ready for I/O */
      /*
       * Find the first channel (if any) that is ready to be read or written.
       */
      for(chan=cp->active_list.head; chan; chan = chan->next) {
	if(chan->active_set == &cp->read_fds ?
       ...
       * channels being watched and call its I/O method. Note that if the
       * I/O method adds the channel back to the set, it will be put at
       * the end of the list. This ensures that no channel can starve
       * out the rest by being lucky enough to be at the front of the list.
       */
      if(chan) {
	if(chan->active_set == &cp->read_fds)
	...
	//	COUT("calling io function on " << chan->fd);
	...
	/*
	 * Check for connection requests on the image monitor-server port.
	 */
      } else if(cp->ims->port >= 0 && FD_ISSET(cp->ims->port, &read_fds)) {
	connect_im_monitor_client(cp, cp->ims);
	/*
	 * Check for connection requests on the real-time controller port.
	 */
      } else if(cp->rtc->port >= 0 && FD_ISSET(cp->rtc->port, &read_fds)) {
	connect_controller(cp, cp->rtc);
	/*
	 * Check for connection requests on the real-time scanner port.
	 */
      } else if(cp->rts->port >= 0 && FD_ISSET(cp->rts->port, &read_fds)) {
	connect_scanner(cp, cp->rts);
	/*
	 * Check for connection requests on the real-time optical camera port.
	 */
 ...
 *  nrs_size         int    The size of the network read buffer including
 *                           the normal message prefix (bytes).
 *  nss_size         int    The size of the network send buffer including
 *                           the normal message prefix (bytes).
...
static CHAN_IO_FN(sock_read_fn)
{
  ...
  case NetReadStr::NET_READ_SIZE:   /* Message partially read before blocking */
  case NetReadStr::NET_READ_DATA:
    add_readable_channel(cp, head);
    break;
  case NetReadStr::NET_READ_DONE:   /* Message completely read */
    /*
    ...
    break;
  case NetReadStr::NET_READ_ERROR:  /* I/O Error */
    close_socket_channel(cp, sock, SOCK_ERROR);
    break;
  case NetReadStr::NET_READ_CLOSED: /* Channel closed by peer */
...
static CHAN_IO_FN(sock_write_fn)
{
  ...
  case NetSendStr::NET_SEND_DATA:  /* Message partially sent before blocking */
    add_writable_channel(cp, head);
    break;
  case NetSendStr::NET_SEND_DONE:		/* Message completely sent */
			      ...
			      PIPE_SENT_FN(*sent_fn))
{
   ...
   * Allocate the pipe.
   */
  pipe->pipe = new gcp::util::PipeQueue();
...
static SOCK_COND_FN(rts_cond_fn)
{
  switch(cond) {
  case SOCK_ERROR:
    lprintf(stderr, "Scanner connection to mediator terminated after error.\n");
    break;
  case SOCK_CLOSE:
    lprintf(stderr, "Scanner connection to mediator was closed.\n");
    break;
  };
}
...
static int connect_scanner(ControlProg *cp, RtScanner *rts)
{
   ...
   */
  {
    ...
    unsigned int arraymap_revision = ARRAYMAP_REVISION;
    unsigned int arraymap_narchive = cp->arraymap->narchive;
    if(net_start_put(net, SCAN_GREETING) ||
       net_put_int(net, 1, &arraymap_revision) ||
       net_put_int(net, 1, &arraymap_narchive) ||
       net_end_put(net))
      return 1;
...
static SOCK_RCVD_FN(rto_rcvd_fn)
{
  ...
  NetBuf *net;         /* The network read buffer */
  unsigned int utc[2];  /* The date of the current image */
  ...
  // Get the resource object of the real-time optical camera
  // connection.
  ...
  if(net_start_get(net, &opcode) ||
     net_get_int(net, 2, utc) ||
     net_get_short(net, 1, &channel) ||
     net_get_int(net, 3, (unsigned int *)actual) ||
     net_get_int(net, 3, (unsigned int *)expected) ||
  ...
  for(i=0; i<IM_MONITOR_CLIENTS; i++) {
	...
	if(net_start_put(client_net, IMC_IMAGE_MSG) ||
	   net_put_short(client_net, 1, &channel) ||
  ...
  if(grabber_save_image(cp, rto->image, utc, channel, actual))
    return 1;
...
static SOCK_COND_FN(rto_cond_fn)
{
  switch(cond) {
  case SOCK_ERROR:
    lprintf(stderr, "Frame grabber connection terminated after error.\n");
    break;
  case SOCK_CLOSE:
    lprintf(stderr, "The frame grabber task disconnected.\n");
    break;
  };
}
...
static MonitorServer *new_MonitorServer(ControlProg *cp, ArrayMap *arraymap)
{
  ...
  for(i=0; i<MONITOR_CLIENTS; i++) {
     ...
     */
    if(i==0) {
      int size1 = net_ArrayMap_size();
      int size2 = net_regs_size(client->regset);
...
static int connect_monitor_client(ControlProg *cp, MonitorServer *ms)
{
  ...
  os << "Accepted monitor-client connection from " << inet_ntoa(client_addr.sin_addr) 
     << "(" << InetAddress::resolveName(inet_ntoa(client_addr.sin_addr), false) << ")";
  lprintf(stdout, "%s.\n", os.str().c_str());
...
static ImMonitorServer *new_ImMonitorServer(ControlProg *cp)
{
   ...
   * Initialize per-client resources.
   *
   * Message buffer must be big enough to accommodate the image as an
   * array of shorts, plus the channel number
   */
  ims->nss_size = NET_PREFIX_LEN + GRABBER_IM_SIZE*2 + 2;
...
static int connect_im_monitor_client(ControlProg *cp, ImMonitorServer *ims)
{
   ...
   */
  std::ostringstream os;
  os << "Accepted image monitor-client connection from " << inet_ntoa(client_addr.sin_addr) 
     << "(" << InetAddress::resolveName(inet_ntoa(client_addr.sin_addr), false) << ")";
  lprintf(stdout, "%s.\n", os.str().c_str());
...
static int connect_control_client(ControlProg *cp, ControlServer *cs)
{
   ...
   */
  std::ostringstream os;
  os << "Accepted control-client connection from " << inet_ntoa(client_addr.sin_addr) 
     << "(" << InetAddress::resolveName(inet_ntoa(client_addr.sin_addr), false) << ")";
  lprintf(stdout, "%s.\n", os.str().c_str());
  ...
  // Finally, tell the grabber thread to update current configuration
  // parameters to all clients.

  if(sendCurrentGrabberConfiguration(cp_Grabber(cp)))
    return 1;
...
static SOCK_RCVD_FN(cc_sock_rcvd_fn)
{
  ...
  case CC_INPUT_CMD:            /* A command to be sent to the scheduler */
    pack_scheduler_command(&msg,
			   netcmd.input.cmd, client->reply->pipe);

    msg.packSerializedData((unsigned char*)scheduler_channel->wbuf);
...
static SOCK_COND_FN(cc_sock_cond_fn)
{
  ...
  switch(cond) {
  case SOCK_ERROR:
    lprintf(stderr,"A control client connection was terminated by an error.\n");
    break;
  case SOCK_CLOSE:
...
static int record_rtc_status(ControlProg *cp, int online)
{
   ...
   */
  pack_scheduler_rtc_state(&msg, online);
  msg.packSerializedData((unsigned char*)cp->thread[CP_SCHEDULER].pipe->wbuf);
...
static SOCK_RCVD_FN(rtc_rcvd_fn)
{
  static NewRtcNetMsg netmsg;    /* A union of message containers */
  std::vector<unsigned char> bytes(netmsg.maxSize());
  unsigned size;          /* The message-type opcode */
  
  // Read the message type and unpack the message.

  if(net_start_get(sock->nrs->net, (int*)&size) ||
     net_get_char(sock->nrs->net, size, &bytes[0]) ||
     net_end_get(sock->nrs->net))
    return 1;

  //  COUT("size = " << size << " maxisze = " << netmsg.maxSize());

  netmsg.deserialize(bytes);

  if(processGenericMsg(cp, sock, netmsg))
    return 1;

  if(processSpecificMsg(cp, sock, netmsg))
    return 1;
...
static int processGenericMsg(ControlProg* cp, SockChan* sock, 
			     NewRtcNetMsg& netmsg)
{
  LoggerMessage logmsg;   /* A logger-thread message container */
  ...
  case NewNetMsg::NET_LOG_MSG:

#if 0
    if(pack_logger_message((LoggerMessage* )
			   cp->thread[CP_LOGGER].pipe->wbuf, 
			   netmsg.msg->log.text,
			   netmsg.msg->log.bad ? LOG_STDERR:LOG_STDOUT))
      return 1;
    
    // Stage the message to be sent.

    add_logger_channel(cp);
    break;
#else
    // I had a PIPE_NOWAIT here previously.  This was causing messages
    // that came in hot on each others' heels to get dropped!

    if(pack_logger_message(&logmsg, netmsg.msg->log.text,
			   netmsg.msg->log.bad ? LOG_STDERR:LOG_STDOUT,
			   netmsg.msg->log.seq, netmsg.msg->log.end) ||
       send_LoggerMessage(cp, &logmsg, PIPE_WAIT) == PIPE_ERROR)
      return 1;
    if(add_readable_channel(cp, &sock->head))
      return 1;
    break;
  ...
  case NewNetMsg::NET_NAV_UPDATE_MSG:

    pack_navigator_rtc_state((NavigatorMessage*)
			     cp->thread[CP_NAVIGATOR].pipe->wbuf, 1);
  ...
  case NewNetMsg::NET_TV_OFFSET_DONE_MSG:
			     ...
			     NewRtcNetMsg& netmsg)
{
  ...
  if(pack_scheduler_rtcnetmsg(&msg, netmsg.msg->getType(), netmsg))
    return 1;
  ...
  return add_scheduler_channel(cp);
}
...
static SOCK_SENT_FN(rtc_sent_fn)
{
  ...
  // Reset the dead-man timeout every time a command is sent to the
  // rtc:

  cp->timeOut_->resetCmdTimeout();
...
static SOCK_COND_FN(rtc_cond_fn)
{
  ...
  switch(cond) {
  case SOCK_ERROR:
    lprintf(stderr,"Control connection to mediator terminated after error.\n");
    break;
  case SOCK_CLOSE:
    lprintf(stderr, "Control connection to mediator was closed.\n");
    break;
  };
  /*
...
static PIPE_RCVD_FN(rtc_pipe_rcvd_fn)
{
  ...
  if(sock->head.read.fd < 0) {
    ...
    if(net_start_put(net, rpc->id) ||
       net_put_int(net, 1, (unsigned int*)&rpc->rtc.antennas) ||
    ...
    cp->timeOut_->registerDeactivateCmdTimeout(rpc->deactivateTimeout);
...
static int connect_controller(ControlProg *cp, RtController *rtc)
{
  ...
  gcp::control::Date utc;          /* The gregorian date (utc) */
...

  {
    ...
    if(net_start_put(net, NET_INIT_CMD) || 
       net_put_int(net, 1, (unsigned int*)&rtcCmd.antennas) ||
...
int sendToCcClients(ControlProg* cp, CcPipeMsg *pmsg)
{
  ...
  for(i=0; i < CONTROL_CLIENTS; i++) {
    ...
    if(cc->sock->head.read.fd > 0)
      waserr |= queue_cc_message(cc->reply->pipe, pmsg);
  }
 ...
 *                      1 - Error.
 */
int queue_cc_message(gcp::util::Pipe *client, CcPipeMsg *pmsg)
{
 ...
 *                         1 - Error.
 */
int queue_rtc_command(ControlProg *cp, RtcNetCmd *rtc, NetCmdId type)
{
...
{
  cp->timeOut_->setCmdIntervalInSeconds(seconds);
...
{
  cp->timeOut_->activateCmdTimeout(activate);
...
{
  cp->timeOut_->setDataIntervalInSeconds(seconds);
  sendDataTimeoutConfiguration(cp, seconds);
}
...
{
  cp->timeOut_->activateDataTimeout(activate);
  sendDataTimeoutConfiguration(cp, activate);
}
...
{
  return cp->dirfileArchive_;
}

bool cp_useModemPager(ControlProg *cp)
{
  return cp->useModemPager_;
}
			    ...
			    bool outOfRange, std::string script)
{
  ...
  if(script.size() > NET_LOG_MAX) {
    lprintf(stderr,"Script name is too long\n");
    return 1;
  }
  ...
  strncpy(msg.body.addPagerReg.script, (char*)script.c_str(), script.size());
  msg.body.addPagerReg.script[script.size()] = '\0';
			     ...
			     unsigned nFrame, bool outOfRange, std::string script)
{
  ...
  try {
    ...
    if(outOfRange) {
      cp->rts->frame->pm->addOutOfRangeMonitorPoint(regSpec, min, max, delta, 
						    nFrame, script);
    } else {
      cp->rts->frame->pm->addInRangeMonitorPoint(regSpec, min, max, delta, 
						 nFrame, script);
    }

    // Tell any connected clients about the new monitor point

    gcp::util::PagerMonitor::RegSpec reg(regSpec, min, max, delta, nFrame, 
					 outOfRange);
...
static void remPagerRegister(ControlProg* cp, std::string regSpec)
{
  ...
  try {
    ...
    gcp::util::PagerMonitor::RegSpec reg(regSpec);
...
void listPager(ControlProg* cp)
{
  ...
  try {
    ...
    for(unsigned iReg=0; iReg < regs.size(); iReg++) 
      sendPagerCondition(cp, PAGECOND_ADD, &regs[iReg]);
  ...
  sendCmdTimeoutConfiguration(cp, cp->timeOut_->cmdTimeoutActive_);
  sendCmdTimeoutConfiguration(cp, (unsigned int)cp->timeOut_->rtcCmdTimeOut_.getSeconds());

  sendDataTimeoutConfiguration(cp, cp->timeOut_->dataTimeoutActive_);
  sendDataTimeoutConfiguration(cp, (unsigned int)cp->timeOut_->rtcDataTimeOut_.getSeconds());
...
{
  if(cp && cp->rtc) {
    CTOUT("printPort: " << cp->rtc->port);
  } else {
    CTOUT("printPort: port is NULL");
  }
}

#define __FILEPATH__ "control/code/unix/control_src/common/navigator.c"
...
#include "gcp/util/common/CenterManager.h"
...
/*
 * A Navigator object is used to maintain the state of the navigator thread.
 */
struct Navigator {
  ...
  int lastScanIndex;        /* The index of the last scan offset sent
			       to the controller. */
  Site site;                /* The location of the telescope */
  CenterManager* phaseManager_; // An object which will keep track of
				// the phase tracking for antennas
  CenterManager* pointingManager_; // An object which will keep track
				   // of the pointing tracking for
				   // antennas
  gcp::util::IersCommunicator*     iersCommunicator_;
};
...
static int nav_update_ephem_source(Navigator *nav, double tt, gcp::control::Source *src,
				   SourceId *id, gcp::util::Tracking::Type type, 
				   unsigned ant, unsigned seq);
static int nav_update_J2000_source(Navigator *nav, double tt, gcp::control::Source *src,
				   SourceId *id, gcp::util::Tracking::Type type, 
				   unsigned ant, unsigned seq);
static int nav_track_fixed_source(Navigator *nav, gcp::control::Source *src, SourceId *id,
				  gcp::util::Tracking::Type type, 
				  unsigned ant, unsigned seq);
static int nav_track_center_source(Navigator *nav, gcp::control::Source *src, SourceId *id,
				   gcp::util::Tracking::Type type, 
				   unsigned ant, unsigned seq);
static int nav_track_J2000_source(Navigator *nav, gcp::control::Source *src, SourceId *id,
				  gcp::util::Tracking::Type type, 
				  unsigned ant, unsigned seq);
static int nav_track_ephem_source(Navigator *nav, gcp::control::Source *src, 
				  SourceId *id,
				  gcp::util::Tracking::Type type, 
				  unsigned ant, unsigned seq);
// Scan functions

static int nav_start_normal_scan(Navigator *nav, Scan *scan, ScanId *id, 
				 unsigned nreps, unsigned seq, bool add);

static int nav_start_continuous_scan(Navigator *nav, Scan *scan, ScanId *id, 
				     unsigned nreps, unsigned seq, bool add);

static int nav_update_normal_scan(Navigator *nav, double tt, Scan *scan,
				  ScanId *id, unsigned nreps, unsigned seq,
				  bool add=false);

static int nav_update_continuous_scan(Navigator *nav, double tt, Scan *scan,
				      ScanId *id, unsigned nreps, unsigned seq,
				      bool add=false);

static int nav_current_scan(Navigator *nav, gcp::control::Scan* scan);
...
/**.......................................................................
 * A function that is called whenever the IersCommunicator downloads a
 * new ephemeris file from the USNO server.  This callback will simply
 * reload the ephemeris file in the control system
 */
static IERS_HANDLER(iersCallbackFn) 
{
  Navigator* nav = (Navigator*) args;
  (void) sc_set_ut1utc_ephemeris(nav->sc, "", (char*)fileName.c_str());
  CTOUT("Called set_ut1utc_ephemeris with fileName = " << fileName);
}

/**.......................................................................
 * A function called to initialize the object that will update (if
 * necessary) the ut1utc ephemeris
 */
static void initializeIersComms(Navigator* nav, std::string fileName) 
{
  // If the object doesn't already exist, then create it now

  if(nav->iersCommunicator_ == 0) {

    CTOUT("UT1-UTC ephemeris updater doesn't exist... creating it now");

    // Tell the comms object to check the ephemeris every hour (3600
    // seconds)

    nav->iersCommunicator_ = new gcp::util::IersCommunicator(fileName, 3600);
    nav->iersCommunicator_->spawn();

    // And add iersCallbackFn as a handler to be called whenever the
    // ephemeris is updated

    nav->iersCommunicator_->addHandler(iersCallbackFn, nav);

    // Else just update the filename if it has changed

  } else {
    CTOUT("UT1-UTC ephemeris updater exists... updating filename");
    nav->iersCommunicator_->updateEphemerisFileName(fileName);
  }
}

/*
 * Record the interval at which to check the need for updating ephemerides
 * etc.  For now, we will update this once a second
 */
#define NAV_WAKEUP_INTERVAL 1000    /* (milli-seconds) */
...
CP_NEW_FN(new_Navigator)
{
  ...
  gcp::control::Source* zenith;  /* The source entry for zenith */
  gcp::control::Source* nadir;   /* The source entry for nadir */
  gcp::control::Source* service; /* The source entry for SERVICE */
  gcp::control::Source* stow;    /* The source entry for STOW */
  gcp::control::Scan *currentScan; /* The scan entry for the current scan */
  gcp::control::Scan *none;     /* The scan entry for none */
  ...
  // Initialize this to null.  Note however, that this will not be
  // initialized until the first time that a ut1utc ephemeris file is
  // specified (otherwise, how do we know what file to update?)

  nav->iersCommunicator_ = 0;
  ...
  //  nav->sc = new_SourceCatalog(true);
  nav->sc = new_SourceCatalog(false);
...
CP_DEL_FN(del_Navigator)
{
  ...
  if(nav) {
    ...
    // Delete the IERS communicator

    if(nav->iersCommunicator_ != 0) {
      delete nav->iersCommunicator_;
      nav->iersCommunicator_ = 0;
    }
...
/*.......................................................................
 * Prepare a "set location" message for subsequent transmission
 * to the navigator thread.
 *
 * Input:
 *  msg  NavigatorMessage *  The message object being prepared for
...
static int nav_handle_message(Navigator *nav, NavigatorMessage *msg)
{
  ...
  case NAV_SCAN_CATALOG:
    ...
    // If a new UT1-UTC ephemeris file has been specified, load it in
    // the control system, and update the information in the IERS
    // communicator object

  case NAV_UT1UTC:
    (void) sc_set_ut1utc_ephemeris(nav->sc, "", msg->body.ut1utc);
#if 0
    // EML commenting auto-update of the ut1utc ephem out until I can
    // figure out how the USNO server has changed to break this

    initializeIersComms(nav, msg->body.ut1utc);
#endif
    break;
  case NAV_SITE:
...
static int nav_check_caches(Navigator *nav)
{
  ...
  DBPRINT(true, Debug::DEBUG3, "Inside nav_check_caches");
  ...
  /*
   * See if the positional parameters of the current scan need to be
   * updated.
   */
  if(w->scan.tmax < tt && w->scan.tmax > 0.0) {
    if(nav_update_scan(nav, tt))
      return 1;
  };
...
static int nav_update_ut1utc(Navigator *nav, double utc, int reset)
{
  ...
  for(i=3-n; i<3; i++) {
    ...
    if(nav->rtc_online && queue_rtc_command(cp, &rtc, NET_UT1UTC_CMD))
      return 1;
  };
...
static int nav_update_eqneqx(Navigator *nav, double tt, int reset)
{
  ...
  for(i=3-n; i<3; i++) {
    ...
    if(nav->rtc_online && queue_rtc_command(cp, &rtc, NET_EQNEQX_CMD))
      return 1;
  };
...
static int nav_update_scan(Navigator *nav, double tt)
{
  ...
  switch(id.type) {
  case SCAN_NORMAL:
    return nav_update_normal_scan(nav, tt, scan, &id, 0, 0);
    break;
  case SCAN_CONTINUOUS:
    return nav_update_continuous_scan(nav, tt, scan, &id, 0, 0);
    break;
...
static int nav_update_source(Navigator *nav, double tt, 
			     gcp::util::Tracking::Type type)
{
  gcp::control::Source* src=0;   /* The current source */
...
static int nav_update_ephem_source(Navigator *nav, double tt, 
				   gcp::control::Source *src, SourceId *id, 
				   gcp::util::Tracking::Type type,
				   unsigned antennas, unsigned seq)
{
  ...
  DBPRINT(true, Debug::DEBUG3, "Inside nav_update_ephem_source()");
  ...
  for(i=3-n; i<3; i++) {
    ...
    rtc.cmd.track.seq  = seq && i==0 ? seq : 0;
    ...
    if(queue_rtc_command(cp, &rtc, NET_TRACK_CMD))
      return 1;
  };
  ...
  if(type & gcp::util::Tracking::TRACK_PHASE) {
    for(i=0; i<3-n; i++) 
      phaseEphemCache[i] = ephemCache[i+n];
    for(i=3-n; i<3; i++) {
				   ...
				   unsigned antennas, unsigned seq)
{
  ...
  rtc.cmd.track.dist = 0;
  rtc.cmd.track.type = type;
  rtc.antennas       = antennas;
  
  // Queue the message to be sent to the real-time-controller.
  
  if(queue_rtc_command(nav->cp, &rtc, NET_TRACK_CMD))
    return 1;
 ...
 *  nreps    unsigned      The number ot times to execute the current scan, 
 *                         or 0 if this is an update of the current scan.
 *  seq      unsigned      The pmac transaction sequence number for a
				  ...
				  bool add)
{
  ...
  // If this is the start of a new scan, reset the last scan index to
  // indicate that we are starting a new scan
  ...
  // The number of milliseconds we want to cover by this send

  unsigned nMs = (SCAN_NET_NPT)*1000;

  // The number of consecutive sends we have to do to for the current
  // scan sampling

  unsigned nMsPerSend = SCAN_NET_NPT*normal->cache.msPerSample;
  unsigned nSend = (nMs/nMsPerSend) + (nMs % nMsPerSend == 0 ? 0 : 1);

  // Iterate sending messages
  
  for(unsigned iSend=0; iSend < nSend && !done; iSend++) {

    // Update the indices of the scan cache that we will send on this
    // command.
    
    if(nav->lastScanIndex < 0) {
      istart = 0;
      istop = SCAN_NET_NPT <= normal->cache.size ? 
	SCAN_NET_NPT-1 : normal->cache.size-1;
    } else {
      if(nav->lastScanIndex == normal->cache.size-1) {
	done = true;
      } else {
	istart = nav->lastScanIndex + 1;
	istop = istart + SCAN_NET_NPT <= normal->cache.size ? 
	  istart + SCAN_NET_NPT-1 : normal->cache.size-1;
      }
    }
    ...
    if(!done) {
      ...
      rtc.cmd.scan.add = add;
      
      // Install fixed members.  These are ignored if seq == 0.
      
      rtc.cmd.scan.istart      = normal->cache.istart;
      rtc.cmd.scan.ibody       = normal->cache.ibody;
      rtc.cmd.scan.iend        = normal->cache.iend;
      rtc.cmd.scan.nreps       = nreps;
      rtc.cmd.scan.msPerSample = normal->cache.msPerSample;
	...
	rtc.cmd.scan.index[i] = normal->cache.offsets[istart+i].index;

	rtc.cmd.scan.flag[i] = normal->cache.offsets[istart+i].flag;

	rtc.cmd.scan.azoff[i] = 
	  (long int)(normal->cache.offsets[istart+i].azoff * dtomas);

	rtc.cmd.scan.eloff[i] = 
	  (long int)(normal->cache.offsets[istart+i].eloff * dtomas);
	...
	return 1;
      ...
      // Set the sequence number to zero to indicate that the next
      // iteration is no longer the start of a new scan
...
/*.......................................................................
 * Send an update of the offsets of a continuous scan to the real-time
 * controller.
 *
 * Input:
 *  nav      Navigator *   The resource object of this thread.
				      ...
				      bool add)
{
  ...
  /*
   * Check that the scan is a continuous scan.
   */
  if(scan->id.type != SCAN_CONTINUOUS) {
    lprintf(stderr, "nav_update_continuous_scan: \"%s\" is not a continuous scan.\n",
	    scan->id.name);
    return 1;
  };
  ...
  unsigned nMsPerSend = SCAN_NET_NPT*continuous->cache.msPerSample;
  unsigned nSend = (nMs/nMsPerSend) + (nMs % nMsPerSend == 0 ? 0 : 1);
  ...
  for(unsigned iSend=0; iSend < nSend && !done; iSend++) {
    ...
    if(nav->lastScanIndex < 0) {
      istart = 0;
      istop = SCAN_NET_NPT <= continuous->cache.size ? 
	SCAN_NET_NPT-1 : continuous->cache.size-1;
    } else {
      if(nav->lastScanIndex == continuous->cache.size-1)
	done = true;
      else {
	istart = nav->lastScanIndex + 1;
	istop = istart + SCAN_NET_NPT <= continuous->cache.size ? 
	  istart + SCAN_NET_NPT-1 : continuous->cache.size-1;
      }
    }
    ...
    if(!done) {
      ...
      rtc.cmd.scan.istart = continuous->cache.istart;
      rtc.cmd.scan.ibody  = continuous->cache.ibody;
      rtc.cmd.scan.iend   = continuous->cache.iend;
      rtc.cmd.scan.nreps  = nreps;
      rtc.cmd.scan.msPerSample = continuous->cache.msPerSample;
	...
	rtc.cmd.scan.index[i] = continuous->cache.offsets[istart+i].index;

	rtc.cmd.scan.flag[i] = continuous->cache.offsets[istart+i].flag;

	rtc.cmd.scan.azoff[i] = 
	  (unsigned)(continuous->cache.offsets[istart+i].azoff * dtomas);

	rtc.cmd.scan.eloff[i] = 
	  (unsigned)(continuous->cache.offsets[istart+i].eloff * dtomas);
      }
...
{
  gcp::control::Source *src;     // The new source 
  SourceInfo info; // Contemporary information about the current source 
  RtcNetCmd rtc;   // The network object to be sent to the real-time
                   // controller task
  int status;      // The return status of pthread functions 
  
  // A sequence number of zero is illegal, both because this is the
  // start value of the tracker sequence before any transactions have
  // been started, and because zero is used to signify an update of
  // the position of the source that the tracker is tracking, not a
  // new track.
  
  if(!seq) {
    lprintf(stderr, "nav_halt_telescope: Illegal zero sequence number.\n");
    return 1;
  };
  ...
  try {
	...
	// Lastly, install this source as the new pointing center for this set
	// of antennas
		   ...
		   bool add)
{
  ...
  switch(id.type) {
  case SCAN_NORMAL:
    if(nav_start_normal_scan(nav, scan, &id, nreps, seq, add))
      return nav_error_while_locked(nav);
    break;
  case SCAN_CONTINUOUS:
    if(nav_start_continuous_scan(nav, scan, &id, nreps, seq, add))
      return nav_error_while_locked(nav);
    break;
  default:
		     ...
		     unsigned antennas, unsigned seq)
{
  ...
  if(seq==0) {
    lprintf(stderr, "nav_track_source: Illegal zero sequence number.\n");
    return 1;
  };
  ...
  try {
	...
	break;
      case SRC_CENTER:
	if(nav_track_center_source(nav, src, &id, type, antennas, seq))
	  return nav_error_while_locked(nav);
	break;
      case SRC_J2000:
				  ...
				  unsigned antennas, unsigned seq)
{
  ...
  rtc.cmd.slew.az     = static_cast<int>(coord.az * rtomas);   /* milli arcseconds */
  rtc.cmd.slew.el     = static_cast<int>(coord.el * rtomas);   
  rtc.cmd.slew.dk     = static_cast<int>(coord.pa * rtomas);   
  rtc.cmd.slew.type   = type;
  rtc.cmd.slew.slewType   = gcp::util::Tracking::TRACK_FIXED;
  rtc.antennas        = antennas;
...
/*.......................................................................
 * This is a private function of nav_track_source(), called on to slew
 * the telescope to a given center source.
 *
 * Input:
 *  nav       Navigator * The resource object of this thread.
 *  src       Source    * The center source to slew to.
 *  id        SourceId  * The identification header of the source.
 *  seq       unsigned    The pmac transaction sequence number to
 *                        record in the NetSlewCmd packet.
 * Output:
				   ...
				   unsigned antennas, unsigned seq)
{
  ...
  if(describe_center_source(src, &coord, &axis_mask))
    return 1;
  ...
  rtc.cmd.slew.seq    = seq;
  rtc.cmd.slew.mask   = axis_mask;
  rtc.cmd.slew.az     = static_cast<int>(coord.az * rtomas); 
  rtc.cmd.slew.el     = static_cast<int>(coord.el * rtomas); 
  rtc.cmd.slew.dk     = static_cast<int>(coord.pa * rtomas); 
  rtc.cmd.slew.type   = type;
  rtc.cmd.slew.slewType   = gcp::util::Tracking::TRACK_CENTER;
  rtc.antennas        = antennas;
 ...
 *                       1 - Error.
 */
static int nav_track_ephem_source(Navigator *nav, gcp::control::Source *src, SourceId *id,
				  gcp::util::Tracking::Type type, 
				  unsigned ant, unsigned seq)
{
  int i;
				 ...
				 unsigned nreps, unsigned seq, bool add)
{
   ...
   * scan.
   */
  if(nav_update_normal_scan(nav, tt, scan, id, nreps, seq, add))
    return 1;
				     ...
				     unsigned nreps, unsigned seq, bool add)
{
   ...
   * scan.
   */
  if(nav_update_continuous_scan(nav, tt, scan, id, nreps, seq, add))
    return 1;
...
static int nav_dispatch_site_to_rtc(Navigator *nav)
{
  ...
  rtc.cmd.site.lon = (double)(nav->site.longitude * rtomas);  /* mill-arcsec */
  rtc.cmd.site.lat = (double)(nav->site.latitude * rtomas);   /* mill-arcsec */
  rtc.cmd.site.alt = (double)(nav->site.altitude * 1000.0);   /* mm */
  rtc.antennas     = AntNum::ANTALL;
...
int nav_lookup_source(Navigator *nav, char *name, int resolve, 
		      SourceId* id)
{
  gcp::control::Source *src=NULL; /* The located source */
...
{
  gcp::control::Scan *scan=NULL; /* The located scan */
  int status;       /* The return status of pthread_mutex_lock() */
  int waserr=0;     /* True if an error occurs while the mutex is held */
  /*
   * Check the arguments.
 ...
 *                      1 - Source not found (no error message).
 */
int nav_get_scan_del(Navigator *nav, char *name, int resolve, ScanId *id, 
		     unsigned nrep, unsigned& ms)
{
  ...
  switch(id->type) {
  case SCAN_NORMAL:
    nstart = (scan->normal.cache.ibody - scan->normal.cache.istart);
    nbody  = (scan->normal.cache.iend - scan->normal.cache.ibody);
    nend   = (scan->normal.cache.size - 1 - scan->normal.cache.iend);
    ms = (nstart + nrep * nbody + nend) * scan->normal.cache.msPerSample;
    break;
  case SCAN_CONTINUOUS:
    nstart = (scan->continuous.cache.ibody - scan->continuous.cache.istart);
    nbody  = (scan->continuous.cache.iend - scan->continuous.cache.ibody);
    nend   = (scan->continuous.cache.size - 1 - scan->continuous.cache.iend);
    ms = (nstart + nrep * nbody + nend) * scan->continuous.cache.msPerSample;
    break;
  default:
    break;
  }
...
int nav_print_scan_info(Navigator *nav, char *name, int resolve, ScanId *id)
{
  ...
  case SCAN_NORMAL:
    ...
    lprintf(stdout, "Npt:  %d\n",scan->normal.cache.size);
    lprintf(stdout, "File: %s\n",scan->normal.file);
    break;
  case SCAN_CONTINUOUS:
    {
      ContinuousScan* continuous = &scan->continuous;
      lprintf(stdout, "Scan: %s\n",id->name);
      lprintf(stdout, "Npt:  \n");
      lprintf(stdout, "\t Start: %d (Index: %d - %d)\n",
	      continuous->cache.ibody - continuous->cache.istart,
	      continuous->cache.istart, continuous->cache.ibody-1);
      
      lprintf(stdout, "\t Body:  %d (Index: %d - %d)\n",
	      continuous->cache.iend - continuous->cache.ibody,
	      continuous->cache.ibody, continuous->cache.iend-1);
      
      lprintf(stdout, "\t End:   %d (Index: %d - %d)\n",
	      continuous->cache.size - continuous->cache.iend,
	      continuous->cache.iend, continuous->cache.size-1);
...
void readSourceCatalog(ControlProg* cp, std::string dir, std::string cat)
{
  ...
  read_SourceCatalog(nav->sc, (char*)dir.c_str(), (char*)cat.c_str());
}

void readScanCatalog(ControlProg* cp, std::string dir, std::string cat)
{
  Navigator* nav = (Navigator* )cp_ThreadData(cp, CP_NAVIGATOR);
  read_ScanCatalog(nav->scanc, (char*)dir.c_str(), (char*)cat.c_str());
}

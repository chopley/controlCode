...
#include "gcp/control/code/unix/libunix_src/common/const.h" // daysec
...
static int read_normal_ephemeris(ScanCatalog *sc, NormalScan *normal);
static int bad_normal_ephemeris(InputStream *input, NormalScan *ephem);
static int read_normal_ephem_entry(InputStream *input, int last_index, 
				   ScanCacheEntry *ee, 
				   NormalScanKeyword* keyword=0, 
				   unsigned* uVal=0);
static int bad_normal_ephem_entry(InputStream *stream, char *column);

/*
 * Continuous scans
 */
static Scan *read_ContinuousScan(ScanCatalog *sc, InputStream *stream,
				 char *name);
static int write_ContinuousScan(ScanCatalog *sc, OutputStream *stream,
				Scan *scan);
static Scan *new_ContinuousScan(ScanCatalog *sc, char *name, char *file);

static int read_continuous_ephemeris(ScanCatalog *sc, ContinuousScan *cont);
static int bad_continuous_ephemeris(InputStream *input, ContinuousScan *ephem);
static int read_continuous_ephem_entry(InputStream *input, int last_index, 
				       ScanCacheEntry *ee, 
				       ContinuousScanKeyword* keyWord,
				       unsigned* uVal=0);
static int bad_continuous_ephem_entry(InputStream *stream, char *column);

/*
 * A scan type for fake scnas, like "current"
 */
...
struct ScanCatalog {
  ...
  FreeList *continuous_mem;  /* Memory for continuous scans */
 ...
 *  return      Scan *   The new scan, or NULL on error.
 */
Scan *add_ContinuousScan(ScanCatalog *sc, char *name, char *file)
{
  Scan *scan;  /* The new scan */
   ...
   * Create the scan and initialize it from the scan file.
   */
  scan = new_ContinuousScan(sc, name, file);
...
static Scan *new_NormalScan(ScanCatalog *sc, char *name, char *file)
{
  ...
  for(i=0; i<SCAN_CACHE_SIZE; i++) {
    ScanCacheEntry* ee = normal->cache.offsets + i;
    ee->index = 0;
   ...
   */
  if(read_normal_ephemeris(sc, normal))
    return del_Scan(sc, scan);
  return scan;
}

/*.......................................................................
 * Allocate a new continuous scan.
 *
 * Input:
 *  sc   ScanCatalog *   The scan catalog to which to add the scan.
 *  name          char *   The name of the scan.
...
{
  Scan *scan;        /* The object to be returned */
  ContinuousScan *continuous; /* A pointer to scan->continuous */
  int i;
   ...
   */
  if(!name || !file) {
    lprintf(stderr, "new_ContinuousScan: NULL argument(s).\n");
    return NULL;
  };
  /*
   * Allocate the container.
   */
  scan = (Scan*)new_FreeListNode("new_ContinuousScan", sc->continuous_mem);
  if(!scan)
    return NULL;
  /*
   ...
   * del_Scan().
   */
  ini_ScanId(scan, SCAN_CONTINUOUS);
  scan->continuous.file         = NULL;
  scan->continuous.cache.size = 0;
   ...
   * Allocate a copy of the file name.
   */
  continuous->file = (char*)malloc(strlen(file) + 1);

  if(!continuous->file) {
    lprintf(stderr,
	    "new_ContinuousScan(%s): Insufficient memory for file name.\n", name);
    return del_Scan(sc, scan);
  };

  strcpy(continuous->file, file);

  /*
   * Clear the cache entries.
   */
  continuous->cache.size = 0;
  for(i=0; i<SCAN_CACHE_SIZE; i++) {
    ScanCacheEntry* ee = continuous->cache.offsets + i;
    ee->index = 0;
    ee->azoff = ee->eloff = ee->dkoff = 0.0;
  };
   ...
   * Read the ephemeris of the scan.
   */
  if(read_continuous_ephemeris(sc, continuous))
    return del_Scan(sc, scan);
  return scan;
}
     ...
     
{
  ...
  if(open_FileInputStream(input, "", normal->file))
    return bad_normal_ephemeris(NULL, normal);
   ...
   */
  cache = normal->cache.offsets;
  
  /*
   * Find the ephemeris entry of the file.
   */
  if(input_skip_white(input, 1, 0)) {
    return bad_normal_ephemeris(input, normal);
  }
  
  // First check for the mspersample keyword  

  NormalScanKeyword keyWord;
  if(read_normal_ephem_entry(input, -1, &cache[0], &keyWord, 
			     &normal->cache.msPerSample))
    return bad_normal_ephemeris(input, normal);
  else if(keyWord != NORM_SCAN_MSPERSAMPLE) /* The file should begin with a
					     START keyword */
    lprintf(stderr, "read_normal_ephemeris: "
	    "The file must begin with a MSPERSAMPLE keyword\n");

  // Read further entries into the cache until either
  // NORMAL_CACHE_SIZE entries have been read or the end of the file
  // is reached.
  
  if(read_normal_ephem_entry(input, -1, &cache[0])) {
    return bad_normal_ephemeris(input, normal);
  } else
    normal->cache.size++;
  ...
  for(nrec=1; nrec < SCAN_CACHE_SIZE && input->nextc!=EOF; nrec++) {
    ...
    if(read_normal_ephem_entry(input, ne[-1].index, ne)) {
      return bad_normal_ephemeris(input, normal);
    } else {
      normal->cache.size++;
    }
  ...
  /*
   * Set the cache pointers appropriately for a normal scan.
   */
  normal->cache.istart = normal->cache.ibody = 0;
  normal->cache.iend   = normal->cache.size;

  return 0;
}

/*.......................................................................
 * This is the private error return function of read_normal_ephemeris().
 *
 * Input:
 ...
 * Output:
 *  return         int    1 (the error return code of read_normal_ephemeris()).
 */
static int bad_normal_ephemeris(InputStream *input, NormalScan *normal)
{
  /*
   * Close the ephemeris file.
   */
  ...
  /*
   * Return the error-return code of read_normal_ephemeris().
   */
  return 1;
}
				   ...
				   unsigned* uVal)
{
   ...
   */
  if(keyWord != 0) 
    *keyWord = NORM_SCAN_NONE;

  /*
   * First see if this is a directive.
   */
  if(isalpha(input->nextc)) {
    if(input_word(input, 0, 1))
      return 1;
    
    /*
     * Identify the scan-type.
     */
    if(strcmp(input->work, "mspersample") == 0) {
      *keyWord = NORM_SCAN_MSPERSAMPLE;
	...
	if(input_uint(input, 0, 0, uVal))
	  return bad_normal_ephem_entry(input, "mspersample");
      }
    }
    ...
    if(input_uint(input, 0, 0, &ee->index))
      return bad_normal_ephem_entry(input, "Index");

    if(input_skip_space(input, 1, 0))
      return 1;

    if(input_sexagesimal(input, 0, &ee->azoff))
      return bad_normal_ephem_entry(input, "sexagesimal AZ offset");

    if(input_skip_space(input, 1, 0))
      return 1;

    if(input_sexagesimal(input, 0, &ee->eloff))
      return bad_normal_ephem_entry(input, "sexagesimal EL offset");

    if(input_skip_space(input, 1, 0))
      return 1;

    if(input_uint(input, 0, 0, &ee->flag))
      return bad_normal_ephem_entry(input, "scan flag");
    ...
    if(last_index >= 0 && ee->index != last_index+1)
...
/*.......................................................................
 * This is a private syntax-error return function for read_normal_ephem_entry().
 *
 * Input:
 ...
 *  return          int    Always 1.
 */
static int bad_normal_ephem_entry(InputStream *input, char *column)
{
  input_error(input, 1, "Bad %s ephemeris field.\n", column);
  input_error(input, 1, "Use: index dd:mm:ss.s dd:mm:ss.s dd:mm:ss.s\n");
  return 1;
}
 ...
 * Input:
 *  sc     ScanCatalog *  The parent catalog of the scan.
 *  scan   ContinuousScan  *  The scan to update.
 *  input  InputStream *  The stream from which to read the ephemeris.
 *
 * Output:
 *  return        int    0 - OK.
 *                       1 - Error.
 */
static int read_continuous_ephemeris(ScanCatalog *sc, ContinuousScan *continuous)
     
{
   ...
   * Are we updating an existing cache?
   */
  update = continuous->cache.size != 0;

  /*
   * Open the ephemeris file.
   */
  input = sc->scan_input;
  if(open_FileInputStream(input, "", continuous->file))
    return bad_continuous_ephemeris(NULL, continuous);
  lprintf(stdout, "%s %s pattern from %s\n",
	  update ? "Updating":"Initializing",
	  continuous->id.name, continuous->file);

  // Get a pointer to the start of the cache buffer.

  cache = continuous->cache.offsets;
  
  // Find the ephemeris entry of the file.

  if(input_skip_white(input, 1, 0))
    return bad_continuous_ephemeris(input, continuous);
  
  // First check for the mspersample keyword  

  if(read_continuous_ephem_entry(input, -1, &cache[0], &keyWord,
				 &continuous->cache.msPerSample))
    return bad_continuous_ephemeris(input, continuous);
  else if(keyWord != CONT_SCAN_MSPERSAMPLE) /* The file should begin with a
					     START keyword */
    lprintf(stderr, "read_continuous_ephemeris: "
	    "The file must begin with a MSPERSAMPLE keyword\n");

  // Now check for the start keyword  

  if(read_continuous_ephem_entry(input, -1, &cache[0], &keyWord))
    return bad_continuous_ephemeris(input, continuous);
  else if(keyWord != CONT_SCAN_START) /* The file should begin with a
					 START keyword */
    lprintf(stderr, "read_continuous_ephemeris: "
	    "The file must contain a START keyword\n");

  
  // Read the first offset out of the file.

  if(read_continuous_ephem_entry(input, -1, &cache[0], &keyWord))
    return bad_continuous_ephemeris(input, continuous);
  else if(keyWord != CONT_SCAN_NONE) {
    lprintf(stderr, "read_continuous_ephemeris: "
	    "No offsets were read for the start sequence.\n");
    return bad_continuous_ephemeris(input, continuous);
  } else {
    continuous->cache.size++;
    haveStart = true;
    continuous->cache.istart = 0;
  }
  
  nrec = 1;
  while(nrec < SCAN_CACHE_SIZE && input->nextc!=EOF) {
    ScanCacheEntry *ne = cache + nrec;
    if(read_continuous_ephem_entry(input, ne[-1].index, ne, &keyWord))
      return bad_continuous_ephemeris(input, continuous);
    else if(keyWord != CONT_SCAN_NONE) {
      switch (keyWord) {
      case CONT_SCAN_START:
	lprintf(stderr, "File should contain only one START directive.\n");
	return bad_continuous_ephemeris(input, continuous);
	break;
      case CONT_SCAN_BODY:
	/* Don't need to check if we have a start record -- we
	   wouldn't have gotten here if we didn't. */
	continuous->cache.ibody = nrec;
	haveBody = true;
	break;
      case CONT_SCAN_END:
	if(!haveBody) {
	  lprintf(stderr, "Found an END directive before a BODY directive was encountered.\n");
	  return bad_continuous_ephemeris(input, continuous);
	} else if(continuous->cache.ibody == nrec) {
	  lprintf(stderr, "No offsets were read for the body sequence\n");
	  return bad_continuous_ephemeris(input, continuous);
	} else {
	  continuous->cache.iend = nrec;
	  haveEnd = true;
	}
	break;
      default:
	lprintf(stderr, "Unrecognized keyword\n");
	return bad_continuous_ephemeris(input, continuous);
	break;
      }
    } else {
  ...
  /*
   * Did we reach the end of the file with no END directive?
   */
  if(!haveEnd) {
    lprintf(stderr, "No END directive found before end of file\n");
    return bad_continuous_ephemeris(input, continuous);
  } else if(continuous->cache.iend == continuous->cache.size-1) {
    lprintf(stderr, "No offsets were read for the END sequence.\n");
    return bad_continuous_ephemeris(input, continuous);
  }
...
/*.......................................................................
 * This is the private error return function of read_continuous_ephemeris().
 *
 * Input:
 *  input  InputStream *  The stream that is connected to the ephemeris
 *                        file (NULL if not yet opened).
 *  ephem  EphemScan *  The incomplete ephemeris.
 * Output:
 *  return         int    1 (the error return code of read_continuous_ephemeris()).
 */
static int bad_continuous_ephemeris(InputStream *input, ContinuousScan *continuous)
{
  /*
   * Close the ephemeris file.
   */
  ...
  /*
   * Return the error-return code of read_continuous_ephemeris().
   */
  return 1;
}
				       ...
				       unsigned* uVal)
{
  ...
  *keyWord = CONT_SCAN_NONE;
  ...
  if(isalpha(input->nextc)) {
	...
	if(input_uint(input, 0, 0, uVal))
	  return bad_continuous_ephem_entry(input, "mspersample");
      }
      
    } else if(strcmp(input->work, "start") == 0) 
      *keyWord = CONT_SCAN_START;
    else if(strcmp(input->work, "body") == 0) 
      *keyWord = CONT_SCAN_BODY;
    else if(strcmp(input->work, "end") == 0) 
      *keyWord = CONT_SCAN_END;
    else
      *keyWord = CONT_SCAN_INVALID;
    ...
    if(input_uint(input, 0, 0, &ee->index))
      return bad_continuous_ephem_entry(input, "Index");
    
    if(input_skip_space(input, 1, 0))
      return 1;
    
    if(input_sexagesimal(input, 0, &ee->azoff))
      return bad_continuous_ephem_entry(input, "sexagesimal AZ offset");
    
    if(input_skip_space(input, 1, 0))
      return 1;
    
    if(input_sexagesimal(input, 0, &ee->eloff))
      return bad_continuous_ephem_entry(input, "sexagesimal EL offset");
    
    if(input_skip_space(input, 1, 0))
      return 1;
    
    if(input_uint(input, 0, 0, &ee->flag))
      return bad_continuous_ephem_entry(input, "scan flags");
    
    if(input_skip_white(input, 1, 0))
      return 1;
  }
  
  // Report an error if a bad keyword was found.

  if(*keyWord == CONT_SCAN_INVALID)
    return input_error(input, 1, "Invalid keyword encountered: %s\n", 
		       input->work);

  // The ephemeris is supposed to be in ascending order of index.

  if(*keyWord == CONT_SCAN_NONE && last_index >= 0 && ee->index != last_index+1)
    return input_error(input, 1, "The ephemeris is not in index order.\n");
...
/*.......................................................................
 * This is a private syntax-error return function for read_continuous_ephem_entry().
 *
 * Input:
 *  input   InputStream *  The stream in which the syntax error was found.
 *  column         char *  The description of the bad element.
 ...
 *  return          int    Always 1.
 */
static int bad_continuous_ephem_entry(InputStream *input, char *column)
{
  input_error(input, 1, "Bad %s ephemeris field.\n", column);
  input_error(input, 1, "Use: index dd:mm:ss.s dd:mm:ss.s dd:mm:ss.s\n");
  input_error(input, 1, "Or:  keyword (START, BODY, END)\n");
  return 1;
}
...
static Scan *del_Scan(ScanCatalog *sc, Scan *scan)
{
  if(scan) {
    ...
    case SCAN_CONTINUOUS:
      if(scan->continuous.file)
	free(scan->continuous.file);
      scan = (Scan*)del_FreeListNode("del_Scan", sc->continuous_mem, scan);
      break;
    case SCAN_ALIAS:
...
ScanCatalog *new_ScanCatalog(void)
{
   ...
   */
  sc->normal_mem = new_FreeList("new_ScanCatalog", sizeof(NormalScan), 32);
  if(!sc->normal_mem)
    return del_ScanCatalog(sc);
  sc->continuous_mem = new_FreeList("new_ScanCatalog", sizeof(ContinuousScan), 32);
  if(!sc->continuous_mem)
    return del_ScanCatalog(sc);
...
ScanCatalog *del_ScanCatalog(ScanCatalog *sc)
{
  ...
  if(sc) {
    ...
    sc->continuous_mem = del_FreeList("del_ScanCatalog", sc->continuous_mem, 1);
  ...
  while(stream->nextc != EOF) {
    ...
    if(input_word(stream, 0, 1)) {
      return input_error(stream, 1,
			 "Missing scan type. Should be NORMAL, CONTINUOUS, or ALIAS.\n");
    };
	...
	return 1;
      break;
    case SCAN_CONTINUOUS:
      if(!read_ContinuousScan(sc, stream, id.name))
	return 1;
      break;
    case SCAN_ALIAS:
...
int output_ScanCatalog(ScanCatalog *sc, OutputStream *stream)
{
  ...
  for(node=sc->head; node; node=node->next) {
	...
	break;
      case SCAN_CONTINUOUS:
	if(write_ContinuousScan(sc, stream, scan))
	  return 1;
	break;
      case SCAN_ALIAS:
...
/*.......................................................................
 * Read the parameters of an continuous scan from an input stream,
 * and add it to the scan catalog.
 *
 * Input:
 *  sc      ScanCatalog *   The catalog to add the scan to.
				 ...
				 char *name)
{
  ...
  /*
   * Read the name of the continuous file into stream->work.
   */
  if(input_skip_space(stream, 1, 0))
    return NULL;
  if(input_word(stream, 0, 0)) {
    input_error(stream, 1, "Missing file name for continuous scan %s.\n",
		name);
    input_error(stream, 1, "Use: CONTINUOUS scan_name file_name\n");
    return NULL;
  };
   ...
   * Allocate the scan container.
   */
  scan = new_ContinuousScan(sc, name, stream->work);
  if(!scan)
    return NULL;
...
/*.......................................................................
 * Write the type-specific parameters of an continuous scan to
 * an output stream.
 *
 * Input:
 *  sc       ScanCatalog  *   The catalog to add the scan to.
...
static int write_ContinuousScan(ScanCatalog *sc, OutputStream *stream,
				Scan *scan)
{
  if(write_OutputStream(stream, scan->continuous.file))
    return 1;
  return 0;
}
		    ...
		    gcp::control::CacheWindow *win)
{
  ...
  win->tmin = tt;
  win->tmax = tt + ((double)(SCAN_NET_NPT/2)) / (daysec); 

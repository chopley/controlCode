#include "gcp/antenna/control/specific/PmacBoard.h"

#include "gcp/util/common/DataArray.h"
#include "gcp/util/common/Exception.h"
#include "gcp/util/common/LogStream.h"
#include "gcp/util/common/Debug.h"
#include "gcp/util/common/PmacFlags.h"
 ...
 * @throw Exception
 */
PmacBoard::PmacBoard(SpecificShare* share, string name, bool simPmac) :  
  Board(share, name), simPmac_(simPmac)
{
  ...
  if(!simPmac_) {
    comms_ = new ServoComms(share, name, simPmac_);

    COUT("About to connect");
    comms_->connect();
    COUT("About to init");
    comms_->initializeAntenna(0);
  }

  COUT("About to init: done");
...
void PmacBoard::commandNewPosition(PmacTarget* pmac)
{
  ...
  COUT("About to command: az = " << az << " el = " << el);

  if(!simPmac_)
    comms_->setAzEl(az, el);
}
...
bool PmacBoard::pmacIsConnected() 
{
  ...
  return comms_->servoIsConnected();
}

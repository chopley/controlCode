#!/usr/bin/env python
'''
This script uses the cbassrx_19apra_pow boff file. It will capture the ADC data into a snap block triggered by a phase switch signal given by the ROACH. I test this using a CW source on the input to the backend- Input the CW source and plot the ADC values- this will show you exactly how long the signal takes to actually flick the phase switch-

In the bof file the following are of interest:
enablePhaseSwitch=1 (turns on the phase switch signals)
phaseFrequency= xx where xx=10 give 1.736MHz, xx=160 gives 186KHz, xx=1280 gives 24.26KHz
demodPhaseSwitch1 = 1 to turn on the digital demodulation
phaseDelay2 = xx where xx is the time it takes from giving the phase switch command to when it is actually implemented on the phase switch (measure using a CW source)
ADCChannel1 is the snap block for Channel 1 etc.
\nAuthor: Charles Copley April 2012.
'''
...
bitstreampower = 'cbassrx_19apra_pow_2012_Apr_19_1348'
...
#		fpga.write_int('acc_len',1000000)
		...
		fpga.write_int('enablePhaseSwitch',1) #fft size
		fpga.write_int('phaseFrequency',1280) #fft size
		fpga.write_int('demodPhaseSwitch1',0) #fft size
		fpga.write_int('demod0_transL',7) #fft size
		fpga.write_int('demod1_transL',6) #fft size
		fpga.write_int('phaseDelay2',30) #fft size
		print 'done'
...
#		plotSpectrum4(Chan1Diff,Chan2Diff,Chan3Diff,Chan4Diff,Chan1Diff,Chan2Diff,Chan3Diff,Chan4Diff,"OnChannelCalibrated")
		...
		while(1):
			adc3 = getSnapCat('ADCChannel1')
			adcVals=numpy.array(adc3)
			numpy.savetxt('ADCSamples.txt',adcVals)

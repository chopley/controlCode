...
def diodeOn():
    fpgaND.write_int('gpio_set4',1)
    #fpga.write_int('gpio1_set',1)
    return 1

def diodeOff():
    fpgaND.write_int('gpio_set4',0)
...
try:
		...
		print('Connecting to server %s on port %i... '%(roachND,katcp_port)),
...
#		fpga.write_int('acc_len',1000000)
		...
		print 'done'
		fpgaND.write_int('cnt_rst',1) 
		fpgaND.write_int('cnt_rst',0) 
		fpgaND.write_int('snap_we',0)
		fpgaND.write_int('snap_we',1)
		fpgaND.write_int('sync_en',1)
		fpgaND.write_int('snap_we',1)
		fpgaND.write_int('ctrl_sw',43690) #fft size
		fpgaND.write_int('acc_sync_delay',10) #fft size
		...
		for i in range(1,intNumber):
			...
			Ch1onSum=Ch1onSum+Channel1On
			Ch1offSum=Ch1offSum+Channel1Off
			Ch2onSum=Ch2onSum+Channel2On
			Ch2offSum=Ch2offSum+Channel2Off
			Ch3onSum=Ch3onSum+Channel3On
			Ch3offSum=Ch3offSum+Channel3Off
			Ch4onSum=Ch4onSum+Channel4On
			Ch4offSum=Ch4offSum+Channel4Off

			filename='OnChannel_'+str(i)
			plotSpectrum4(Ch1onSum/i,Ch2onSum/i,Ch3onSum/i,Ch4onSum/i,Ch1offSum/i,Ch2offSum/i,Ch3offSum/i,Ch4offSum/i,filename)
		...
		[coffs1r,coffs1i,coffs2r,coffs2i,coffs3r,coffs3i,coffs4r,coffs4i]=gainCalibration(Ch1onSum/intNumber,Ch2onSum/intNumber,Ch3onSum/intNumber,Ch4onSum/intNumber)

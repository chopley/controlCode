...
/////////////////////////////////
	...
	//accumulations=78125-2;
	accumulations=20000-2;
	fwrite(&accumulations, 4, 1, fp);
	fclose(fp);
...
//	fwrite(&writeVal, 4, 1, fp);
	...
	//
		...
		gainCorrection(&tempPack,10);
		tempSum=0;
		for(i=5;i<=31;i++){
			tempSum+=tempPack.data_ch0evenMSB[i];	
		}
		fprintf(fpwrite,"%d,%d,%d,%f,%d,%d\n",tempSum,acc_new,ndstate,tottimediff,accumulations);
		printf("%d,%d,%d,%f,%d,%d\n",tempSum,acc_new,ndstate,tottimediff,accumulations);
		///////////////////////////

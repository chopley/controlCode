...
//this is a thread that runs when we are in the polarisation mode- you can define commands from the control system and data feed back in here
	...
	{	printf("Waiting for Connection\n");
		...
		FD_ZERO(&acceptfds); //zero the select file descriptor
		FD_SET(childfd, &acceptfds); //and add childfd to the set
		tv.tv_sec=2; //2 second timeout
		tv.tv_usec=0; //do this to zero this value
		n=select(childfd+1,&acceptfds,NULL,NULL,&tv) //and set up the select timeout
		if(FD_ISSET(childfd,&acceptfds)){
			childfd=accept(sock,(struct sockaddr *) &cliAddr,&cliLen);
			printf("Got connection\n");
		}
		if (n <0){
		...
		//		printf("recv 1a blocking %d %d\n",n,childfd);
				...
				returnDataSize=0; //zero the return data size
				FD_ZERO(&readfds); //zero the select file descriptor
				FD_SET(childfd, &readfds); //and add childfd to the set
				tv.tv_sec=2; //2 second timeout
				tv.tv_usec=0; //do this to zero this value
				n=select(childfd+1,&readfds,NULL,NULL,&tv) //and set up the select timeout

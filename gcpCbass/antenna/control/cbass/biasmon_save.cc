...
LnaBiasMonitor::LnaBiasMonitor() : Board(), SpawnableTask<LnaBiasMonitorMsg>(0) {
  ...
  try{
    ...
    COUT("Could not connect to LabJack LNA");
  };
}
...
LnaBiasMonitor::LnaBiasMonitor(SpecificShare* share, string name, bool spawn) : Board(share, name), SpawnableTask<LnaBiasMonitorMsg>(spawn)   {
  ...
  COUT("DONE WITH CONNECT IN CONSTRUCTOR");
...
{
  if(connected_==false){
    COUT("LabJack not connected");
    reConnectCounter_++;
    //    COUT("RECONNCOUNTE: " << reConnectCounter_);
    if(reConnectCounter_ < 10){
      return;
    } else {
      reConnectCounter_ = 0;
      COUT("Labjack has been disconnected for over 10 seconds");
      COUT("Will try to reconnect");
       try{
	connect();
      } catch(Exception& err) {
	COUT("Error in reconnect");
	return;
      }
       if(connected_ == false){
	COUT("Re-connection failed");
	return;
      };
    };
  };
  ...
  if(voltages[0]==0){
    COUT("LabJack became disconnected");
    disconnect();
  }

 ...
 */
#define TRACKING_LIMIT_ASEC 120
...
ServoCommandSa ServoCommsSa::issueCommand(ServoCommandSa::Request req, std::vector<float>& values)
{
  ...
  while(tryRead){
    ...
    case 2:
      ...
      numTimeOut++;
		COUT("Timeout in ServocommsSa.cc");
      break;
...
void ServoCommsSa::initializeAntenna()
{
  ...
  if(sim_) {
    ...
    initializationPartOne_ = true;
    COUT("NOT INITIALIZING BECAUSE WE'RE SIMULATING");
    return;
  }
  ...
  */
  ...
  CTOUT("drive lid open: " << driveLidsOpen);
  ...
  CTOUT("INIT PART ONE SHOULD BE COMPLETE");
...
void ServoCommsSa::fillUtc(gcp::util::TimeVal& currTime)
{
  ...
  // incrememnt second by a negative number does not work on this machine.
  TimeVal onesec;
  onesec.setSeconds(1);
  currTime -= onesec;
  //  currTime.incrementNanoSeconds(-1.0);
  //  CTOUT("IN FILLUTC 1a2");
  // CTOUT("FASTSERVOUTC.SIZE() " << fastServoUtc.size());
...
bool ServoCommsSa::readPosition(AxisPositions* axes, Model* model)
{
  ...
  if(share_) {
    ...
    float azErrServo = command.responseValue_[0];
    if(azErrServo > 270000){
      azErrServo -= 360000;
    } else if (azErrServo < -270000) {
      azErrServo += 360000;
    };
    float azErr = azErrServo*cos(elDegrees*3.14159/180);  // in mDeg

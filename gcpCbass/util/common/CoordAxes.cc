#define __FILEPATH__ "util/common/CoordAxes.cc"
...
{
  if(regAxes != 0) {
    nEl_.resize(regAxes->nEl_.size());
    for(unsigned i=0; i < nEl_.size(); i++)
      nEl_[i] = regAxes->nEl_[i];
  }
}
...
std::vector<Range<unsigned> > CoordAxes::getRanges(CoordRange coordRange)
{
  ...
  if(!rangeIsValid(coordRange)) {
    COUT("Range " << coordRange << " does not contain valid data");
...
bool CoordAxes::rangeIsValid(CoordRange& range)
{
  ...
  if(!range.isValid()) {
    ReportError("Range is not valid: " << range);
    return false;
  }

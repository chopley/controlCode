	...
	for(unsigned int iboard=0; iboard < regmap->nboard_; iboard++) {
	  ...
	  if(boardIsPresent(brd) && fm.boardIsPresent(brd)) {
		...
		if(blockIsPresent(blk) && fm.blockIsPresent(blk)) {
		  ...
		  if(strcmp(blk->name_, "features")==0 && strcmp(aregmap->name, "array")==0) {
...
{
  int regOffset = aregmap->byteOffsetInArrayMap(archivedOnly_);
 ...
 */
int ArrayMapDataFrameManager::
byteOffsetInFrameOf(std::string regmapName, std::string brdName)
{
  ArrRegMap*   arm = arrayMap_->findArrRegMap(regmapName);
  RegMapBoard* brd = find_RegMapBoard(arm->regmap, brdName);

  int regMapOffset = arm->byteOffsetInArrayMap(archivedOnly_);
  int boardOffset  = brd->byteOffsetInRegMap(archivedOnly_);
 
  if(regMapOffset < 0 || boardOffset < 0)
    return -1;
  
  return byteOffsetInFrameOfData() + regMapOffset + boardOffset;
}
...
/**.......................................................................
 * Copy the contents of a BoardDataFrameManager to the appropriate
 * place in the array map.
 */
void ArrayMapDataFrameManager::writeBoard(BoardDataFrameManager& fm, 
					  bool lockFrame)
{
  ArrRegMap* aregmap = arrayMap_->findArrRegMap(fm.regmap()->name);
  ...
  try {
    ...
    if(fm.archivedOnly() == archivedOnly_) {
      ...
      unsigned iStartDest = byteOffsetInFrameOf(fm.regmap()->name, fm.board()->name);
      ...
      //      COUT("start src  = " << iStartSrc); 
      //      COUT("start dest = " << iStartDest); 
      //      COUT("nbyte      = " << fm.sizeInBytesOfData());
      ...
      RegMapBoard* srcBrd = fm.board();

      for(unsigned int iboard=0; iboard < regmap->nboard_; iboard++) {
	RegMapBoard* brd = regmap->boards_[iboard];
	
	// Only copy the requested board
	
	if(boardIsPresent(brd) && strcmp(brd->name, srcBrd->name)==0) {
					 ...
					 bool lockFrame)
{
  ...
  try {
    ...
    if(fm.archivedOnly() == archivedOnly_) {
      ...
      unsigned iStartSrc = byteOffsetInFrameOf(fm.regmap()->name, fm.board()->name);
      ...
      COUT("start src  = " << iStartSrc);
      COUT("start dest = " << iStartDest);
      COUT("nbyte      = " << aregmap->nByte(archivedOnly_));
#endif
...
{
  return frame_->getPtr(byteOffsetInFrameOf(desc.regMap(), desc.block(), &desc.getRangePtr()->startCoord()),

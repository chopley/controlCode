...
{
  DBPRINT(true, Debug::DEBUG9, this << ": terminating comm sequence");
...
void Communicator::processClientMessage()
{
  ...
  client_->concatenateString(os_);

  COUT("Inside processClientMessage: os = " << os_.str());
...
void Communicator::sendNextString(void)
{
  COUT("Sending: " << *sentStringIter_);

			  ...
			  CoordRange* range)
{
  ...
  for(axisRange.reset(); !axisRange.isEnd(); ++axisRange) {
	...
	ierr = ms_get_short(ms_, desc, &val->val.data_.s, &coordRange);
	break;
      case MonitorDataType::FM_USHORT:
	ierr = ms_get_ushort(ms_, desc, &val->val.data_.us, &coordRange);
	break;
...
MonitorDataType::FormatType Monitor::readFormat(InputStream* stream)
{
  ...
  if(stream->nextc != EOF && stream->nextc != '\n') {
    if(input_skip_space(stream, 1, 0)) {
      errStr.appendMessage(true, "Error reading format");
      throw Error(errStr);
    } 
  }
...
MonitorDataType::FormatType Monitor::parseFormat(std::string str) 
{
  for(unsigned iformat=0; iformat < nFormat_; iformat++)
    if(formats[iformat].format.compare(str.c_str())==0)
      return formats[iformat].type;
...
MonitorDataType::FormatType Monitor::formatOf(RegDescription& reg, MonitorDataType::FormatType format)
{
  ...
  switch (gcp::util::DataType::typeOf(reg.block())) {
  case gcp::util::DataType::UCHAR:
    return MonitorDataType::FM_UCHAR;
    break;
  case gcp::util::DataType::CHAR:
    return MonitorDataType::FM_CHAR;
    break;
  case gcp::util::DataType::USHORT:
    return MonitorDataType::FM_USHORT;
    break;
  case gcp::util::DataType::SHORT:
    return MonitorDataType::FM_SHORT;
    break;
  case gcp::util::DataType::UINT:
    return MonitorDataType::FM_UINT;
    break;
  case gcp::util::DataType::INT:
    return MonitorDataType::FM_INT;
    break;
  case gcp::util::DataType::ULONG:
    return MonitorDataType::FM_ULONG;
    break;
  case gcp::util::DataType::LONG:
    return MonitorDataType::FM_LONG;
    break;
  case gcp::util::DataType::FLOAT:
    return MonitorDataType::FM_FLOAT;
    break;
  case gcp::util::DataType::DOUBLE:
    return MonitorDataType::FM_DOUBLE;
    break;
  case gcp::util::DataType::DATE:
    return MonitorDataType::FM_DATE;
    break;
  case gcp::util::DataType::COMPLEX_FLOAT:
...
void Monitor::printArrayMap(bool archivedOnly)
{
  if(ms_->arraymap)
    ms_->arraymap->print(archivedOnly);;
}

std::vector<std::string> Monitor::getArrayMapRegisters(bool archivedOnly)
{
  std::vector<std::string> regs;

  if(ms_->arraymap)
    regs = ms_->arraymap->getRegisters(archivedOnly);

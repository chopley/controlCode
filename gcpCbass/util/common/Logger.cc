...
LOG_HANDLER_FN(*Logger::logHandler_) = 0;
LOG_HANDLER_FN(*Logger::errHandler_) = 0;
LOG_HANDLER_FN(*Logger::stdoutPrintFn_) = 0;
LOG_HANDLER_FN(*Logger::stderrPrintFn_) = 0;
...
void Logger::write(const std::string& message, bool isError, bool dispatch, bool log)
{
  ...
  int incr   = gcp::control::NET_LOG_MAX - (prefix_.length()+1);
...
{
  if((prefix.length()) >= gcp::control::NET_LOG_MAX)

		...
		CoordRange* range)
{
  ...
  try {
    ...
    MonitorPoint* monitorPoint = findMonitorPoint(regMapName, boardName, blockName, range, false);

    // Don't re-create the monitor point if it already exists
...
addMonitorPoint(char* boardName, char* blockName, CoordRange* range)
{
  ...
  try {
    ...
    MonitorPoint* monitorPoint = findMonitorPoint(boardName, blockName, range, false);
 ...
 */
std::ostream& gcp::util::operator<<(std::ostream& os, MonitorPointManager& mp)
{
  std::map<std::string, MonitorPoint*>::iterator iMap;
...
RegMapBlock* MonitorPointManager::findReg(std::string regmap, std::string board, 
					  std::string block)
{
  return arrMapFm_->findReg(regmap, board, block);
}

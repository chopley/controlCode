#define __FILEPATH__ "util/common/NetCmdHandler.cc"
  ...
  NetHandler()
{
  ...
  // Create unassigned TCP/IP network streams for control
  
  // Make this long enough to accomodate the network prefix, the
  // antenna bitmask, and the command itself.

  nrs_->setBuffer(NULL, 
		  NET_PREFIX_LEN+sizeof(unsigned)+net_max_obj_size(&rtc_cmd_table));
...
{ 
  ...
  //DBPRINT(true, Debug::DEBUGANY, "Packing a command: opcode = " << opcode << ", antenna = " << rtc->antennas);

  nss_->startPut((int)opcode);
  nss_->putInt(1, &rtc->antennas);
...
void NetCmdHandler::readNetCmd()
{
  ...
  if(Debug::debugging(Debug::DEBUG8)) {
    if(lastReadNetCmd_.opcode_ == gcp::control::NET_INIT_CMD)
      DBPRINT(true, Debug::DEBUG8, "Got an init command");
  }
  
  nrs_->getInt(1, &lastReadNetCmd_.rtc_.antennas);

#define __FILEPATH__ "util/common/RegCoordRange.cc"

#include "gcp/util/common/RegCoordRange.h"
 ...
 */
ostream& 
gcp::util::operator<<(ostream& os, RegCoordRange& range)
{
  os << "Current element = " << range.iElCurrent_ << endl;
  os << "Current slot = " 
     << (range.iSlotOffset_ < 0 ? -1 : range.iSlotOffset_ + (int)range.iElCurrent_);
  return os;
}

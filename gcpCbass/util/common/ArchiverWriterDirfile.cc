 ...
 */
ArchiverWriterDirfile::ArchiverWriterDirfile(unsigned nBuffer)
{
  initialize(0, 0, nBuffer);
}
				       ...
				       unsigned nBuffer) 
{
  ...
  mapRegisters(arrayMap, basePtr, nBuffer);
}
					 ...
					 unsigned nBuffer)
{
  ...
  if(arrayMap != 0 && basePtr != 0) {  
    
    for(std::vector<ArrRegMap*>::iterator 
	  iRegMap=arrayMap->regmaps.begin(); 
	iRegMap != arrayMap->regmaps.end(); iRegMap++) {
      
      ArrRegMap* arrRegMap = *iRegMap;
      
      for(std::vector<RegMapBoard*>::iterator 
	    iBoard=arrRegMap->regmap->boards_.begin();
	  iBoard != arrRegMap->regmap->boards_.end(); iBoard++) {

	RegMapBoard* board = *iBoard;
	for(std::vector<RegMapBlock*>::iterator 
	      iBlock=board->blocks.begin();
	    iBlock != board->blocks.end(); iBlock++) {
	  
	  insert(*iBlock, arrRegMap, basePtr, nBuffer);
	}
      }
    }
  }
 ...
 */
int ArchiverWriterDirfile::openArcfile(std::string dirname)
{
  int waserr=0;
...
int ArchiverWriterDirfile::Register::write()
{
  ...
  // type
    ...
    for(unsigned iEl=0; iEl < nEl_; iEl++) {
      ...
      memcpy((void*)(destBase+iEl*nBytePerEl_), (void*)&mjd, dSize);
    }

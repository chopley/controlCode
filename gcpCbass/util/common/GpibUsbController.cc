...
{
  DBPRINT(true, Debug::DEBUG7, "Here RR 0");
  std::string response = readString();
  DBPRINT(true, Debug::DEBUG7, "Here RR 1");
			  ...
			  GPIB_RESPONSE_HANDLER(*handler), bool block, void* retVal)
{
  ...
  DBPRINT(true, Debug::DEBUG7, "Here 3 (" << pthread_self() << ")");
...
GPIB_RESPONSE_HANDLER(GpibUsbController::checkHandler)
{
  ...
  //  COUT("Inside checkHandler: " << eos << " resp = " << gpib->response_.str());

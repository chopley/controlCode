 ...
 */
void MonitorPoint::checkHandlers(bool isSim, gcp::util::AxisRange& range)
{
...
{
  if(handler_ != 0) 
    handler_(arg_, conditionWasSatisfied, message_, script_);
}
...
registerConditionHandler(MONITOR_CONDITION_HANDLER(*handler), 
			 ...
			 bool persistent,
			 gcp::util::CoordRange* coordRange)
{

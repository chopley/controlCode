...
{
  if(throwFn_ == 0)
    defaultThrowFn(os, fileName, lineNumber, functionName, isErr, isSimple, 
		   isSysErr);
  else
...
{
  if(reportFn_ == 0)
    defaultReportFn(os, fileName, lineNumber, functionName, isErr, isSimple, 
		    isSysErr);
  else
    reportFn_(os, fileName, lineNumber, functionName, isErr, isSimple, 
	      isSysErr);
}
...
{
  if(logFn_ == 0)
    defaultLogFn(os, fileName, lineNumber, functionName, isErr, isSimple, 
		 isSysErr);
  else
...
ERR_HANDLER_FN(ErrHandler::defaultThrowFn)
{
  ...
  errStr.initLogStreamMessage(fileName, lineNumber, functionName, true, 
			      !isSimple);

  // If this is a system error, append the system message to it

  if(isSysErr)
    errStr.appendSysLogStreamError(os.str(), fileName, lineNumber, 
  ...
  throw gcp::util::Exception(errStr, fileName, lineNumber, true);
}
...
ERR_HANDLER_FN(ErrHandler::defaultReportFn)
{
  ...
  errStr.initLogStreamMessage(fileName, lineNumber, functionName, isErr, 
			      !isSimple);

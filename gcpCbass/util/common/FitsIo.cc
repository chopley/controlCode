#define __FILEPATH__ "util/common/FitsIo.cc"

#include "gcp/util/common/FitsIo.h"
...
void FitsIo::writeFileHeaderBody(FILE* fp)
{
  ...
  sprintf(kval,"%f", obsRa_.degrees());
  putPhdu("OBSRA",kval,"", fp);
  sprintf(kval,"%f", obsDec_.degrees());
...
void FitsIo::writeVisibilityDataBody(double* vis, double* date, double* uvw, FILE* fp)
{
  ...
  for(unsigned iFrame=0; iFrame < nFrame_; iFrame++) {
    ...
    for(unsigned iBaseline=0; iBaseline < nBaseline_; iBaseline++) {
      ...
      // buffer and write it to the UVF file.  Store the UVW as light 
      // travel time in seconds across the projected 
      //
      // cvel is in meters/s!, hence the factor of 100

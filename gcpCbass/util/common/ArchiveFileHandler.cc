...
#include "gcp/control/code/unix/libunix_src/common/arraytemplate.h"
 ...
 * Output Operator.
 */
std::ostream& gcp::util::operator<<(std::ostream& os, ArchiveFileHandler& obj)
{
...
void ArchiveFileHandler::readTimestamps()
{
  ...
  COUT("Elapsed time = " << diff.getFractionalTimeInSeconds());
}
 ...
 */
void ArchiveFileHandler::readSize()
{
  ...
  if(net_start_get(net, &opcode) ||
     ...
     net_get_int(net, 1, (unsigned int* )&nrsSize) ||
     net_end_get(net)) {
 ...
 */
gcp::control::MsReadState ArchiveFileHandler::readNextMsg()
{

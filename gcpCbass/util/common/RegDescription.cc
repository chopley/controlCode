#define __FILEPATH__ "util/common/RegDescription.cc"
  ...
  archivedOnly_(archivedOnly)
{
  ...
  arrayMap_    = 0;
  arrayMap_    = (arrayMap != 0 ? alias_ArrayMap(arrayMap) : alias_ArrayMap(arrayMapBase_.arrayMap()));
...
RegDescription& RegDescription::operator=(const RegDescription& desc)
{
...
RegDescription& RegDescription::operator=(RegDescription& desc)
{
  ...
  if(this != &desc) // Check for assignment to self
  {
    clear();
 ...
 *                          1 - Error.
 */
ostream& gcp::util::operator<<(std::ostream& os, RegDescription& desc)
{
  ...
  if(!desc.axes_.rangeIsValid(desc.range_)) {
    COUT("Slot numbers out of range");
...
bool RegDescription::rangeIsValid(CoordRange& range)
{
  if(axes_.rangeIsValid(range)) {
    ...
    COUT("Inside RegDescription::rangeIsValid 1");
    return 0;
  }
}
...
bool RegDescription::rangeIsValid(CoordRange* range)
{
  if(axes_.rangeIsValid(range)) {
    ...
    COUT("Inside RegDescription::rangeIsValid 2");
    return 0;
  }
}
...
void RegDescription::setCoord(Coord& coord)
{
  ...
  if(axes_.rangeIsValid(range))
    ...
    COUT("Invalid coordinate for register: " << *this);

			  ...
			  struct tm& refdate, double& reftime)
{
  ...
  setTime(static_cast<long unsigned int>(reftime + (3600 * date.tm_hour) + (60 * date.tm_min) + date.tm_sec), usec, 0);
...
//handle error if timing is wrong for some reason
  if(tVal.seconds_ <= seconds_){
	//if the value is ok at this stage we reset the error counter
	ErrorCounter_=0;
}
  else if(tVal.seconds_ > seconds_){
	//else if the value is rubbish, then we set the tVal seconds to current timeand increment the error counter up to a maximum of 10 consecutive errors
	tVal.seconds_=seconds_;
	ErrorCounter_++;
	CTOUT("Results will be negative"<<ErrorCounter_);
	if(ErrorCounter_>10){
...
/*
  if(tVal.seconds_ > seconds_){
	CTOUT("Error in the TimeVal operator");
       throw Error("TimeVal::operator-: result will be negative.\n");
        }*/

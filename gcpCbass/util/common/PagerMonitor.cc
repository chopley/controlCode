...
PagerMonitor::PagerMonitor(ArrayDataFrameManager* source) :
  source_(source), dest_(*source), mpManager_(&dest_), handler_(0), arg_(0)
{
}
  ...
  
{
  ...
  return addMonitorPoint(regSpec, 
			 ...
			 dMin, dMax, delta, nFrame, true, script);
}
				     ...
				     std::string script)
{
  ...
  return addMonitorPoint(regSpec, 
			 ...
			 dMin, dMax, delta, nFrame, false, script);
}
			      ...
			      std::string script)
{
  ...
  for(unsigned iDesc=0; iDesc < regDescs.size(); iDesc++) {
    ...
    mp->registerConditionHandler(handler_, arg_, os.str(), script, condition, 
				 true, desc.getRangePtr());
    
    RegSpec regSpec(source_, desc, mp, outOfRange, mpManager_);

    // And add it to our vector of monitor points.  But only if it
...
PagerMonitor::RegSpec::RegSpec(ArrayDataFrameManager* sourceFm, 
			       ...
			       bool outOfRange, MonitorPointManager& mpm)
{
  block_        = mpm.findReg(desc.regMapName(), desc.boardName(), desc.blockName());
  range_        = desc.range();
  mp_           = mp;
...
void PagerMonitor::RegSpec::operator=(const RegSpec& regSpec)
{
  *this = (RegSpec&) regSpec;
}

void PagerMonitor::RegSpec::operator=(RegSpec& regSpec)
{
...
std::ostream& gcp::util::operator<<(std::ostream& os, PagerMonitor::RegSpec& rs) 
{
  if(rs.block_ == 0) {

    os << "Error: block is NULL";

  } else {

    if(rs.block_->brd_ == 0) {
      os << "Error: board is NULL";
    } else {
      os << rs.block_->brd_->name << "." << rs.block_->name_;
    }
  }
  return os;
}
...
std::ostream& gcp::util::operator<<(std::ostream& os, PagerMonitor& pm)
{
  os << pm.mpManager_;
  return os;
}

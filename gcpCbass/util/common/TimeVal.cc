 ...
 *  nanoseconds passed.
 */
TimeVal::TimeVal(unsigned int seconds, unsigned int microSeconds, 
 ...
 *  nanoseconds passed.
 */
TimeVal::TimeVal(unsigned int seconds, unsigned int nanoSeconds)
{
...
void TimeVal::setTime(double seconds)
{
  ...
  unsigned int intSec = (unsigned int) seconds;
  unsigned int intNanoSec = (unsigned int)((seconds - intSec) * NSEC_PER_SEC);

  setTime(intSec, intNanoSec);
}

void TimeVal::setDateTime(string time_string, string format, 
			  struct tm& refdate, double& reftime)
{
  struct tm date;

  char *end=strptime(time_string.c_str(), format.c_str(), &date);
  

  if(!end){
    ThrowError(time_string 
	  << "\" was poorly formatted! " 
	  << "I Expected a string of the format \"" 
	  << format << "\"!");
  }
  ...
  if((unsigned)(end-time_string.c_str())< time_string.size() &&
     *end=='.'){
  ...
  // If reference date is not today, calculate the time for the start
  // of today and cache it in refdate and reftime
  
  if((date.tm_mday != refdate.tm_mday) ||
     (date.tm_mon != refdate.tm_mon) ||
     (date.tm_year != refdate.tm_year))
  {
    refdate = date;
    refdate.tm_hour = 0;
    refdate.tm_min = 0;
    refdate.tm_sec = 0;
    gcp::control::Date thisDate;
    gcp::control::init_Date(&thisDate, date.tm_year + 1900, date.tm_mon + 1, date.tm_mday, 0, 0, 0, 0);
    reftime = (gcp::control::date_to_mjd_utc(&thisDate) - MJD0) * SEC_PER_DAY;
  }
  
  setTime(static_cast<unsigned int>(reftime + (3600 * date.tm_hour) + (60 * date.tm_min) + date.tm_sec), usec, 0);
  
  string timezone;
  iss >> timezone;

  if(timezone.size() && (timezone.substr(0,3) != "UTC"))
  {
    ThrowError("TimveVal(string) - Only UTC timestamps supported. Received " << timezone.substr(0,3));
  }
}

TimeVal::TimeVal(string time_string, string format, struct tm& refdate, double& reftime)
{
  setDateTime(time_string, format, refdate, reftime);
}

TimeVal::TimeVal(string time_string, string format)
{
  struct tm          refdate;
  double             reftime;
  
  refdate.tm_year = 0; // set year in the past to force update of reference date and time
 ...
 * Set the time.
 */
void TimeVal::setTime(unsigned int seconds, unsigned int microSeconds, 
		      unsigned int nanoSeconds)
{
  // Compute the number of nanoseconds in the sub-second
  // specifications.

  unsigned int nsec = microSeconds * NSEC_PER_USEC + nanoSeconds;
 ...
 * Set the time.
 */
void TimeVal::setTime(unsigned int seconds, unsigned int nanoSeconds)
{
...
void TimeVal::setTime(const struct timespec& timeSpec)
{
  ...
  setSeconds((unsigned int)timeSpec.tv_sec);
...
void TimeVal::setTime(const struct timeval& timeval)
{
  ...
  setSeconds((unsigned int)timeval.tv_sec);
...
{
  unsigned int incSec = (unsigned int)seconds;
  unsigned int incNanoSec = (unsigned int)((seconds - incSec) * NSEC_PER_SEC);

  unsigned int sec  = seconds_     + incSec;
  unsigned int nsec = nanoSeconds_ + incNanoSec;
...
{
  int incNanoSec = (int)nanoSeconds;

  int sec  = seconds_;
  int nsec = nanoSeconds_ + incNanoSec;
...
{
  int nanoSec = (int)milliSeconds * NSEC_PER_MSEC;
 ...
 */
unsigned int TimeVal::getTimeInMicroSeconds()
{
 ...
 */
unsigned int TimeVal::getTimeInMilliSeconds()
{
  unsigned mSec;
 ...
 * Return the total time as integer nanoseconds.
 */
unsigned int TimeVal::getTimeInNanoSeconds()
{
 ...
 */
unsigned int TimeVal::getSeconds()
{
  return seconds_;
}
 ...
 *
 */
unsigned int TimeVal::getMilliSeconds(bool round)
{
  double mSec = (double)(nanoSeconds_) / NSEC_PER_MSEC;

  if(!round)
    return (unsigned int) nanoSeconds_ / NSEC_PER_MSEC;
  else {
    return (unsigned int) rint(mSec);
  }
}
 ...
 */
unsigned int TimeVal::getMicroSeconds()
{
 ...
 */
unsigned int TimeVal::getNanoSeconds()
{
  return nanoSeconds_;
}
...
{
  unsigned int dsec  = seconds_     - timeVal_.tv_sec;
  unsigned int dnsec = nanoSeconds_ - timeVal_.tv_usec * NSEC_PER_USEC;
 ...
 */
unsigned int TimeVal::getElapsedTimeInNanoSeconds()
{
  unsigned int dsec  = seconds_     - timeVal_.tv_sec;
  unsigned int dnsec = nanoSeconds_ - timeVal_.tv_usec * NSEC_PER_USEC;
 ...
 */
unsigned int TimeVal::getMjdDays()
{
  return mjdDays_;
}
 ...
 */
unsigned int TimeVal::getMjdSeconds()
{
  return mjdSeconds_;
}
 ...
 */
unsigned int TimeVal::getMjdMilliSeconds(bool round)
{
  ...
  if(!round)
    ...
    return mjdSeconds_ * MSEC_PER_SEC + (unsigned int) rint(mSec);
}

/**.......................................................................
 * Return the mjd nanoseconds corresponding to this time.
 */
unsigned int TimeVal::getMjdNanoSeconds()
{
...
void TimeVal::setMjd(double mjd)
{
  ...
  unsigned int seconds = 
    (unsigned int)((mjd - days) * SEC_PER_DAY);

  // Get the truncated integer nanoSeconds into the second

  unsigned int nSec    = 
    (unsigned int)(((mjd - days) * SEC_PER_DAY - seconds) * NSEC_PER_SEC);
 ...
 */
void TimeVal::setMjd(unsigned int days, unsigned int seconds, 
		     unsigned int nanoSeconds)
{
  unsigned int sec = (days - MJD0) * SEC_PER_DAY + seconds;
 ...
 * Set the time, as an MJD.
 */
void TimeVal::setMjd(unsigned int days, unsigned int milliSeconds)
{
  unsigned int mjdSec = milliSeconds / MSEC_PER_SEC;
  unsigned int mjdNanoSec = (milliSeconds - mjdSec * MSEC_PER_SEC) * 
    NSEC_PER_MSEC;

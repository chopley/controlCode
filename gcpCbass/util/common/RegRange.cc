#define __FILEPATH__ "util/common/RegRange.cc"
		   ...
		   bool archivedOnly, ArrayMap* arrayMap)
{
  if(arrayMap != 0)
    ...
    arrayMap_ = alias_ArrayMap(arrayMapBase_.arrayMap());
 ...
 */
ostream& 
gcp::util::operator<<(ostream& os, RegRange& range)
{
  if(range.iEl_ < 0)
    os << "Not initialized";
  else {
...
void RegRange::checkValidity()
{
  if(iEl_ < 0) {
    LogStream errStr;
    errStr.appendMessage(true, "Current element is not set\n");
    throw Error(errStr);
  }
}

...
Date::Date(unsigned iYear, unsigned dayInYear, unsigned hour, unsigned min, unsigned sec)
{
  setTo(iYear, dayInYear, hour, min, sec);
}
...
void Date::setTo40mWxString(std::string date)
{
  String str(date);

  // Date is separate by the '/' character

  unsigned int year  = str.findNextStringSeparatedByChars(" /").toInt();
  unsigned int month = str.findNextStringSeparatedByChars(" /").toInt();
  unsigned int day   = str.findNextStringSeparatedByChars(" /").toInt();
...
void Date::setToCbassWxString(std::string date)
{
  String str(date);
...
void Date::setTo(unsigned iYear, unsigned dayInYear, unsigned hour, unsigned min, unsigned sec)
{
  setMjd(calToMjd(1, 1, iYear, hour, min, sec) + (dayInYear - 1));
}

unsigned Date::dayInWeek(double mjd)
{
  double jd = mjd + 2400000.5;

  COUT("JD = " << std::setw(12) << std::setprecision(12) << jd);

  unsigned ujd = (unsigned) jd;
  double jdnoon = ujd + 0.5;
  ...
  COUT("UJD = " << std::setw(12) << std::setprecision(12) << ujd);

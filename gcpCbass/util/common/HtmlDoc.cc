...
void HtmlDoc::writeHtmlCommandSynopsisFile(std::string& dir, ScriptCmd& cmd)
{
  std::ostringstream os;
...
void HtmlDoc::writeHtmlFunctionSynopsisFile(std::string& dir, ScriptCmd& cmd)
{
  std::ostringstream os;
  os << dir << "/commands/" << cmd.name_ << "_Synopsis.html";
  std::ofstream fout(os.str().c_str(), ios::out);
  ...
  fout << "<dt><span class=declaration>"
       << "<a class=plain href=\"javascript:loadPage('../dataTypes/" << cmd.retType_.dataTypeName_ << "_Index\
.html')\">"
